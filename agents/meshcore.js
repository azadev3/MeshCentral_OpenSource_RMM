...
if (process.platform == 'win32' && require('user-sessions').isRoot()) {
    ...
    // Check to see if we are the Installed Renovar Agent Service, if we are, make sure we can run in Safe Mode
    var svcname = process.platform == 'win32' ? 'Renovar Agent' : 'meshagent';
    try {
        ...
        require('service-manager').manager.installService(
            {
                ...
                displayName: "Renovar Agent Diagnostic Service",
                description: "Renovar Agent Diagnostic Service",
    ...
    if (typeof data == 'object') {
        ...
        switch (data.action) {
            ...
            case 'uninstallagent':
                ...
                var agentName = process.platform == 'win32' ? 'Renovar Agent' : 'meshagent';
                try {
            ...
            case 'meshToolInfo':
                ...
                if (data.tag == 'install') {
                    data.func = function (options, success) {
                        sendConsoleText('Download of RenovarCentral Assistant ' + (success ? 'succeed' : 'failed'));
                        if (success) {
                            // TODO: Install & Run
                        }
                    }
...
function terminal_promise_connection_resolved(term)
{
    ...
    if (this.ws.httprequest.consent && (this.ws.httprequest.consent & 2))
    {
        ...
        var notifyTitle = "RenovarCentral";
...
function tunnel_kvm_end()
{
    ...
    if (this.desktop.kvm.connectionCount == 0)
    {
        ...
        // try { require('toaster').Toast('RenovarCentral', 'Remote Desktop Control Ended.'); } catch (ex) { }
        ...
        if (this.httprequest.protocol == 0) { // 1 = Terminal (admin), 2 = Desktop, 5 = Files, 6 = PowerShell (admin), 7 = Plugin Data Exchange, 8 = Terminal (user), 9 = PowerShell (user), 10 = FileTransfer
            ...
            if (this.httprequest.protocol == 10) {
                ...
                if (this.httprequest.consent && (this.httprequest.consent & 16))
                {
                    ...
                    var consentTitle = 'RenovarCentral';
...
function processConsoleCommand(cmd, args, rights, sessionid) {
    try {
        ...
        switch (cmd) {
            ...
            case 'assistant':
                if (process.platform == 'win32') {
                    // Install RenovarCentral Assistant on this device
                    ...
                    response = "RenovarCentral Assistant is not supported on this platform.";
                }
                break;
            case 'userimage':
            ...
            */
            ...
            case 'toast': {
                if (args['_'].length < 1) { response = 'Proper usage: toast "message"'; } else {
                    if (require('MeshAgent')._tsid == null) {
                        require('toaster').Toast('RenovarCentral', args['_'][0]).then(sendConsoleText, sendConsoleText);
                    }
                    else {
                        require('toaster').Toast('RenovarCentral', args['_'][0], require('MeshAgent')._tsid).then(sendConsoleText, sendConsoleText);
                    }
                }
                break;
            }
...
function agentUpdate_Start(updateurl, updateoptions) {
    ...
    if (agentUpdate_Start._selfupdate != null) {
        ...
        if (require('MeshAgent').ARCHID == null && updateurl == null) {
            ...
            if (name == null) { name = (process.platform == 'win32' ? 'Renovar Agent' : 'meshagent'); } // This is an older agent that doesn't expose the service name, so use the default
            try {
                var s = require('service-manager').manager.getService(name);
                if (!s.isMe()) {
                    if (process.platform == 'win32') { s.close(); }
            ...
            if (updateurl == null) { options.path = ('/renovar?id=' + require('MeshAgent').ARCHID); sendConsoleText('Downloading update from: ' + options.path, sessionid); }
            options.rejectUnauthorized = false;
            options.checkServerIdentity = function checkServerIdentity(certs) {
                // If the tunnel certificate matches the control channel certificate, accept the connection
                try { if (require('MeshAgent').ServerInfo.ControlChannelCertificate.digest == certs[0].digest) return; } catch (ex) { }
