...
function DownloadAgentBinary(path, ID)
{
    ...
    var downloadUri = 'https://' + options.host + ':' + options.port + '/renovar?id=' + (ID != null ? ID : getARCHID());
...
function giveup()
{
    sendServerLog('Diagnostic: Unable to diagnose Renovar Agent');
    finished();
}
function finished()
{
...
function start()
{
        ...
        DownloadAgentBinary('agent_temporary.bin').then(function ()
        {
            ...
            try
            {
                var agent = require('service-manager').manager.installService(
                    {
                        name: process.platform == 'win32' ? 'Renovar Agent' : 'meshagent',
                        target: 'meshagent',
                        description: 'Mesh Central Agent v2 Background Service',
                        displayName: 'Renovar Agent v2 Background Service',
    ...
    if(s!=null)
    {
        // Renovar Agent Installation Found
        sendServerLog('Diagnostic: Renovar Agent Service => ' + (s.isRunning() ? 'RUNNING' : 'NOT-RUNNING'));
        if(s.isRunning())
        {
            finished();
        }
        ...
        {
            sendServerLog('Diagnostic: Attempting to start Renovar Agent');
            s.start();
