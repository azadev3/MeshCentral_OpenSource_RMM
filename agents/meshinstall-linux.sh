...
DownloadAgent() {
  ...
  wget $url/renovar?id=$machineid {{{wgetoptionshttps}}}-O ./meshagent || curl {{{curloptionshttps}}}--output ./meshagent $url/renovar?id=$machineid

  # If it did not work, try again using http
  if [ $? != 0 ]
  then
    url=${url/"https://"/"http://"}
    wget $url/renovar?id=$machineid {{{wgetoptionshttp}}}-O ./meshagent || curl {{{curloptionshttp}}}--output ./meshagent $url/renovar?id=$machineid
  fi
  ...
  then
    ...
    echo "Unable to download agent at: $url/renovar?id=$machineid."
  fi
}
