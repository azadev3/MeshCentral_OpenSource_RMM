/**
* @description RenovarCentral Intel AMT manager
...
module.exports.CreateAmtManager = function (parent) {
    ...
    function removeDevice(nodeid) {
        ...
        // If a 802.1x profile is active with RenovarCentral Satellite, notify Satellite of the removal
    ...
    obj.HandleEvent = function (source, event, ids, id) {
        switch (event.action) {
            ...
            case 'meshchange': {
                ...
                // TODO: If a device changes to a device group that does not have a 802.1x policy, we may need to tell RenovarCentral Satellite to remove the 802.1x profile.
                ...
                switch (event.subaction) {
                    ...
                    case '802.1x-Profile-Completed': {
                        ...
                        if ((event.response == null) || (typeof event.response != 'object')) {
                            ...
                            devFound.consoleMsg("RenovarCentral Satellite could not create a 802.1x profile for this device.");
    ...
    function attemptTlsSyncEx(stack, name, responses, status) {
        ...
        // Find the current TLS certificate & RenovarCentral root certificate
        ...
        dev.amtstack.BatchEnum(null, objQuery, function (stack, name, responses, status) {
            ...
            if (wirelessConfig) {
                ...
                if (parent.config.domains[dev.domainid].amtmanager.wifiprofiles != null) {
                    ...
                    // Check if any other WIFI profiles require a 802.1x request to RenovarCentral Satellite
            ...
            // See if we need to ask RenovarCentral Satellite for a new 802.1x profile
            if (newNetAuthProfileRequested && (typeof srvNetAuthProfile.satellitecredentials == 'string')) {
                // Credentials for this 802.1x profile are provided using RenovarCentral Satellite
                // Send a message to Satellite requesting a 802.1x profile for this device
                dev.consoleMsg("Requesting 802.1x credentials for " + netAuthStrings[srvNetAuthProfile.authenticationprotocol] + " from RenovarCentral Satellite...");
                ...
                const netAuthTimeoutFunc = function netAuthTimeout() {
                    ...
                    if (dev.netAuthSatReqId != null) {
                        ...
                        netAuthTimeout.dev.consoleMsg("RenovarCentral Satellite did not respond in time, 802.1x profile will not be set.");
                ...
                // No need to call RenovarCentral Satellite for a 802.1x profile, so configure everything now.
        ...
        if (dev.netAuthCredentials.certificate) {
            ...
            const f = function AddCertificateResponse(stack, name, response, status) {
                if ((status != 200) || (response.Body['ReturnValue'] != 0)) {
                    ...
                    AddCertificateResponse.dev.consoleMsg("Setting RenovarCentral Satellite 802.1x profile...");
            ...
            dev.consoleMsg("Setting RenovarCentral Satellite 802.1x profile...");
    ...
    function attempt8021xSyncEx(dev, devNetAuthData) {
        ...
        if (wiredConfig) {
            ...
            if (netAuthProfile['Enabled']) {
                ...
                // If we have a RenovarCentral Satellite profile, use that
                if (dev.netAuthCredentials != null) {
        ...
        if (wirelessConfig) {
            ...
            for (var i in profilesToAdd) {
                ...
                if (([5, 7, 32768, 32769].indexOf(profileToAdd.authentication)) >= 0) {
                    ...
                    // If we have credentials from RenovarCentral Satelite, use that
                    if (dev.netAuthCredentials != null) {
                        const srvNetAuthProfile2 = dev.netAuthCredentials;
                        if (srvNetAuthProfile2.username && (srvNetAuthProfile2.username != '')) { netAuthProfile['Username'] = srvNetAuthProfile2.username; }
                        if (srvNetAuthProfile2.password && (srvNetAuthProfile2.password != '')) { netAuthProfile['Password'] = srvNetAuthProfile2.password; }
