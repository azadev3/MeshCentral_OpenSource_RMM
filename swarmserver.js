/**
* @description RenovarCentral v1 legacy Swarm Server, used to update agents and get them on MeshCentral2
...
module.exports.CreateSwarmServer = function (parent, db, args, certificates) {
    ...
    obj.server.listen(args.swarmport, function () { console.log('RenovarCentral Legacy Swarm Server running on ' + certificates.CommonName + ':' + args.swarmport + '.'); obj.parent.updateServerState('swarm-port', args.swarmport); }).on('error', function (err) { console.error('ERROR: RenovarCentral Swarm Server server port ' + args.swarmport + ' is not available.'); if (args.exactports) { process.exit(); } });
    loadMigrationAgents();
