...
/**
* @description RenovarCentral command line tool
...
if (args['_'].length == 0) {
    console.log("MeshCtrl performs command line actions on a RenovarCentral server.");
        ...
        case 'help': {
            if (args['_'].length < 2) {
                ...
                switch (args['_'][1].toLowerCase()) {
                    ...
                    case 'serverinfo': {
                        console.log("Get information on the RenovarCentral server, Example usages:\r\n");
                    ...
                    case 'listusers': {
                        console.log("List the account on the RenovarCentral server, Example usages:\r\n");
                    ...
                    case 'listusersessions': {
                        console.log("List active user sessions on the RenovarCentral server, Example usages:\r\n");
                        console.log("  MeshCtrl ListUserSessions");
                        console.log("  MeshCtrl ListUserSessions --json");
                        break;
                    }
                    case 'listusergroups': {
                        console.log("List user groups on the RenovarCentral server, Example usages:\r\n");
                    ...
                    case 'devicemessage': {
                        ...
                        console.log("\r\nOptional arguments:\r\n");
                        console.log("  --title [title]        - Messagebox title, default is \"RenovarCentral\".");
                        break;
                    }
                    case 'devicetoast': {
                        ...
                        console.log("  --msg [message]        - The message to display.");
                        console.log("\r\nOptional arguments:\r\n");
                        console.log("  --title [title]        - Toast title, default is \"RenovarCentral\".");
                        break;
                    }
                    case 'report': {
...
function serverConnect() {
    ...
    ws.on('open', function open() {
        ...
        switch (settings.cmd) {
            ...
            case 'userinfo': { break; }
            case 'listusers': { ws.send(JSON.stringify({ correlationid: settings?.correlationid || settings.cmd?.correlationid || args?.correlationid || nodeid?.correlationid, action: 'users', responseid: 'meshctrl' })); break; }
            case 'listusersessions': { ws.send(JSON.stringify({ correlationid: settings?.correlationid || settings.cmd?.correlationid || args?.correlationid || nodeid?.correlationid, action: 'wssessioncount', responseid: 'meshctrl' })); break; }
            case 'removeallusersfromusergroup':
            case 'listusergroups': { ws.send(JSON.stringify({ correlationid: settings?.correlationid || settings.cmd?.correlationid || args?.correlationid || nodeid?.correlationid, action: 'usergroups', responseid: 'meshctrl' })); break; }
            case 'listdevicegroups': { ws.send(JSON.stringify({ correlationid: settings?.correlationid || settings.cmd?.correlationid || args?.correlationid || nodeid?.correlationid, action: 'meshes', responseid: 'meshctrl' })); break; }
            case 'listusersofdevicegroup': { ws.send(JSON.stringify({ correlationid: settings?.correlationid || settings.cmd?.correlationid || args?.correlationid || nodeid?.correlationid, action: 'meshes', responseid: 'meshctrl' })); break; }
            case 'listdevices': {
                if (args.details) {
                    // Get list of devices with lots of details
                    ws.send(JSON.stringify({ correlationid: args?.correlationid || nodeid?.correlationid || setting?.correlationid || settings.cmd?.correlationid, action: 'getDeviceDetails', type: (args.csv) ? 'csv' : 'json' }));
                } else if (args.group) {
                    ws.send(JSON.stringify({ correlationid: args?.correlationid || nodeid?.correlationid || setting?.correlationid || settings.cmd?.correlationid, action: 'nodes', meshname: args.group, responseid: 'meshctrl' }));
                } else if (args.id) {
                    ws.send(JSON.stringify({ correlationid: args?.correlationid || nodeid?.correlationid || setting?.correlationid || settings.cmd?.correlationid, action: 'nodes', meshid: args.id, responseid: 'meshctrl' }));
                } else {
                    ws.send(JSON.stringify({ correlationid: args?.correlationid || nodeid?.correlationid || setting?.correlationid || settings.cmd?.correlationid, action: 'meshes' }));
                    ws.send(JSON.stringify({ correlationid: args?.correlationid || nodeid?.correlationid || setting?.correlationid || settings.cmd?.correlationid, action: 'nodes', responseid: 'meshctrl' }));
                }
                break;
            }
            case 'listevents': {
                ...
                var cmd = null;
                if (args.userid) {
                    cmd = { correlationid: args?.correlationid || nodeid?.correlationid, action: 'events', user: args.userid, responseid: 'meshctrl' };
                } else if (args.id) {
                    cmd = { correlationid: args?.correlationid || nodeid?.correlationid, action: 'events', nodeid: args.id, responseid: 'meshctrl' };
                } else {
                    cmd = { correlationid: args?.correlationid || nodeid?.correlationid, action: 'events', responseid: 'meshctrl' };
                }
            ...
            case 'removeuser': {
                var userid = args.userid;
                if ((args.domain != null) && (userid.indexOf('/') < 0)) { userid = 'user/' + args.domain + '/' + userid; }
                ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'deleteuser', userid: userid, responseid: 'meshctrl' }));
                break;
            }
            case 'addusergroup': {
                var op = { correlationid: nodeid?.correlationid || args?.correlationid, action: 'createusergroup', name: args.name, desc: args.desc, responseid: 'meshctrl' };
                if (args.domain) { op.domain = args.domain; }
                ws.send(JSON.stringify(op));
                break;
            }
            case 'removeusergroup': {
                ...
                ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'deleteusergroup', ugrpid: ugrpid, responseid: 'meshctrl' }));
                break;
            }
            case 'addtousergroup': {
                var ugrpid = args.groupid;
                ...
                if (args.userid != null) {
                    var userid = args.userid;
                    if ((args.domain != null) && (userid.indexOf('/') < 0)) { userid = 'user/' + args.domain + '/' + userid; }
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'addusertousergroup', ugrpid: ugrpid, usernames: [userid.split('/')[2]], responseid: 'meshctrl' }));
                    break;
                }

                if ((args.id != null) && (args.id.startsWith('user/'))) {
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'addusertousergroup', ugrpid: ugrpid, usernames: [args.id.split('/')[2]], responseid: 'meshctrl' }));
                    break;
                }
                ...
                if (args.meshid != null) {
                    ...
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'addmeshuser', meshid: meshid, userid: ugrpid, meshadmin: rights, responseid: 'meshctrl' }));
                    break;
                }

                if ((args.id != null) && (args.id.startsWith('mesh/'))) {
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'addmeshuser', meshid: args.id, userid: ugrpid, meshadmin: rights, responseid: 'meshctrl' }));
                    break;
                }
                ...
                if (args.nodeid != null) {
                    ...
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'adddeviceuser', nodeid: nodeid, userids: [ugrpid], rights: rights, responseid: 'meshctrl' }));
                    break;
                }

                if ((args.id != null) && (args.id.startsWith('node/'))) {
                    ws.send(JSON.stringify({ correlationid: nodeid?.correlationid || args?.correlationid, action: 'adddeviceuser', nodeid: args.id, userids: [ugrpid], rights: rights, responseid: 'meshctrl' }));
                    break;
                }
            ...
            case 'agentdownload': {
                ...
                u += 'id=' + args.type + '&meshagents=' + args.id;
            ...
            case 'devicemessage': {
                ws.send(JSON.stringify({ action: 'msg', type: 'messagebox', nodeid: args.id, title: args.title ? args.title : "RenovarCentral", msg: args.msg, responseid: 'meshctrl' }));
                break;
            }
            case 'devicetoast': {
                ws.send(JSON.stringify({ action: 'toast', nodeids: [args.id], title: args.title ? args.title : "RenovarCentral", msg: args.msg, responseid: 'meshctrl' }));
                break;
            }
            case 'report': {
                var reporttype = 1;
        ...
        switch (data.action) {
            ...
            case 'authcookie': { // SHELL, UPLOAD, DOWNLOAD
                if ((settings.cmd == 'shell') || (settings.cmd == 'upload') || (settings.cmd == 'download')) {
                    ...
                    ws.send(JSON.stringify({correlationid: args?.correlationid || node?.correlationid || parent?.correlationid || command?.correlationid, action: 'msg', nodeid: args.id, type: 'tunnel', usage: 1, value: '*/meshrelay.ashx?p=' + protocol + '&nodeid=' + args.id + '&id=' + id + '&rauth=' + data.rcookie, responseid: 'meshctrl' }));
    ...
    if (settings.cmd == 'shell') {
        ...
        settings.tunnelws.on('message', function (rawdata) {
            ...
            if (settings.tunnelwsstate == 1) {
                ...
                if (termSize != null) { settings.tunnelws.send(JSON.stringify({ correlationid: settings.cmd?.correlationid || settings?.correlationid, ctrlChannel: '102938', type: 'options', cols: termSize[0], rows: termSize[1] })); }
                ...
                process.stdout.on('resize', function () {
                    var termSize = null;
                    if (typeof process.stdout.getWindowSize == 'function') { termSize = process.stdout.getWindowSize(); }
                    if (termSize != null) { settings.tunnelws.send(JSON.stringify({ correlationid: settings.cmd?.correlationid || settings?.correlationid, ctrlChannel: '102938', type: 'termsize', cols: termSize[0], rows: termSize[1] })); }
                });
            }
        });
        ...
        settings.tunnelws.on('message', function (rawdata) {
            if (settings.tunnelwsstate == 1) {
                ...
                settings.tunnelws.send(JSON.stringify({correlationid: settings.cmd?.correlationid || settings?.correlationid || data?.correlationid, action: 'upload', reqid: 'up', path: args.target, name: require('path').basename(args.file), size: settings.uploadSize }));
            }
        });
...
function displayDeviceInfo(sysinfo, lastconnect, network, nodes) {
    ...
    if (node.agent) {
        ...
        if ((node.agent != null) && (node.agent.id != null) && (node.agent.ver != null)) {
            ...
            output["Renovar Agent"] = str; outputCount++;
        }
        if ((node.conn & 1) != 0) {
        ...
        if (outputCount > 0) { info["Renovar Agent"] = output; }
    }
...
// Read the Renovar Agent error log and index it.
