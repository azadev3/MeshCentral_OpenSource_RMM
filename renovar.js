/**
* @description RenovarCentral main module
...
if (process.argv[2] == '--launch') { try { require('appmetrics-dash').monitor({ url: '/', title: 'RenovarCentral', port: 88, host: '127.0.0.1' }); } catch (ex) { } }
...
function CreateMeshCentralServer(config, args) {
    ...
    obj.swarmserver = null;     // Swarm server, this is used only to update older RenovarCentral v1 agents
    ...
    obj.meshAgentBinaries = {};       // Renovar Agent Binaries, Architecture type --> { hash:(sha384 hash), size:(binary size), path:(binary path) }
    ...
    if (obj.platform == 'win32') {
        ...
        obj.servicelog = new eventlogger('RenovarCentral');
    }

    // Start the Meshcentral server
    obj.Start = function () {
        var i;
        try { require('./pass.js').hash('test', function () { }, 0); } catch (ex) { console.log('Old version of node, must upgrade.'); return; } // TODO: Not sure if this test works or not.
        ...
        if ((obj.args.help == true) || (obj.args['?'] == true)) {
            console.log('RenovarCentral v' + getCurrentVersion() + ', remote computer management web portal.');
            ...
            if ((obj.platform == 'win32') || (obj.platform == 'linux')) {
                ...
                console.log('   --install/uninstall               Install RenovarCentral as a background service.');
                console.log('   --start/stop/restart              Control RenovarCentral background service.');
                console.log('');
            ...
            console.log('');
            console.log('Server recovery commands, use only when RenovarCentral is offline.');
        ...
        if (obj.args.hashpassword) { require('./pass.js').hash(obj.args.hashpassword, function (err, salt, hash, tag) { console.log(salt + ',' + hash); process.exit(); }); return; }
        ...
        if (obj.platform == 'linux') {
            if (obj.args.install == true) {
                // Install RenovarCentral in Systemd
                console.log('Installing RenovarCentral as background Service...');
                ...
                require('child_process').exec('which node', {}, function (error, stdout, stderr) {
                    ...
                    require('child_process').exec('echo \"' + config + '\" | sudo tee ' + systemdConf, {}, function (error, stdout, stderr) {
                        ...
                        require('child_process').exec('sudo systemctl enable meshcentral.service', {}, function (error, stdout, stderr) {
                            if ((error != null) && (error != '')) { console.log('ERROR: Unable to enable RenovarCentral as a service: ' + error); process.exit(); return; }
                            if (stdout.length > 0) { console.log(stdout); }
                            console.log('Starting service...');
                            require('child_process').exec('sudo systemctl start meshcentral.service', {}, function (error, stdout, stderr) {
                                if ((error != null) && (error != '')) { console.log('ERROR: Unable to start RenovarCentral as a service: ' + error); process.exit(); return; }
                                if (stdout.length > 0) { console.log(stdout); }
                                console.log('Done.');
                            });
                        });
                ...
                // Uninstall RenovarCentral in Systemd
                console.log('Uninstalling RenovarCentral background service...');
                var systemdConf = null;
                if (require('fs').existsSync('/etc/systemd/system')) { systemdConf = '/etc/systemd/system/meshcentral.service'; }
                else if (require('fs').existsSync('/lib/systemd/system')) { systemdConf = '/lib/systemd/system/meshcentral.service'; }
                else if (require('fs').existsSync('/usr/lib/systemd/system')) { systemdConf = '/usr/lib/systemd/system/meshcentral.service'; }
                ...
                require('child_process').exec('sudo systemctl stop meshcentral.service', {}, function (err, stdout, stderr) {
                    if ((err != null) && (err != '')) { console.log('ERROR: Unable to stop RenovarCentral as a service: ' + err); }
                    if (stdout.length > 0) { console.log(stdout); }
                    ...
                    require('child_process').exec('sudo systemctl disable meshcentral.service', {}, function (err, stdout, stderr) {
                        if ((err != null) && (err != '')) { console.log('ERROR: Unable to disable RenovarCentral as a service: ' + err); }
                        if (stdout.length > 0) { console.log(stdout); }
                        console.log('Removing config file...');
                        require('child_process').exec('sudo rm ' + systemdConf, {}, function (err, stdout, stderr) {
                            if ((err != null) && (err != '')) { console.log('ERROR: Unable to delete RenovarCentral config file: ' + err); }
                            console.log('Done.');
                        });
                    });
                });
                ...
                // Start RenovarCentral in Systemd
                require('child_process').exec('sudo systemctl start meshcentral.service', {}, function (err, stdout, stderr) {
                    if ((err != null) && (err != '')) { console.log('ERROR: Unable to start RenovarCentral: ' + err); process.exit(); return; }
                    console.log('Done.');
                });
                return;
            } else if (obj.args.stop == true) {
                // Stop RenovarCentral in Systemd
                require('child_process').exec('sudo systemctl stop meshcentral.service', {}, function (err, stdout, stderr) {
                    if ((err != null) && (err != '')) { console.log('ERROR: Unable to stop RenovarCentral: ' + err); process.exit(); return; }
                    console.log('Done.');
                });
                return;
            } else if (obj.args.restart == true) {
                // Restart RenovarCentral in Systemd
                require('child_process').exec('sudo systemctl restart meshcentral.service', {}, function (err, stdout, stderr) {
                    if ((err != null) && (err != '')) { console.log('ERROR: Unable to restart RenovarCentral: ' + err); process.exit(); return; }
                    console.log('Done.');
                });
                return;
            }
        ...
        if ((obj.platform == 'win32') && (obj.service != null)) {
            // Build RenovarCentral parent path and Windows Service path
            ...
            if (((obj.args.xinstall == true) || (obj.args.xuninstall == true) || (obj.args.start == true) || (obj.args.stop == true) || (obj.args.restart == true))) {
                ...
                const svc = new obj.service({ name: 'RenovarCentral', description: 'RenovarCentral Remote Management Server', script: servicepath, env: env, wait: 2, grow: 0.5 });
                svc.on('install', function () { console.log('RenovarCentral service installed.'); svc.start(); });
                svc.on('uninstall', function () { console.log('RenovarCentral service uninstalled.'); process.exit(); });
                svc.on('start', function () { console.log('RenovarCentral service started.'); process.exit(); });
                svc.on('stop', function () { console.log('RenovarCentral service stopped.'); if (obj.args.stop) { process.exit(); } if (obj.args.restart) { console.log('Holding 5 seconds...'); setTimeout(function () { svc.start(); }, 5000); } });
                svc.on('alreadyinstalled', function () { console.log('RenovarCentral service already installed.'); process.exit(); });
                svc.on('invalidinstallation', function () { console.log('Invalid RenovarCentral service installation.'); process.exit(); });
            ...
            if (obj.args.install == true) {
                console.log('Installing RenovarCentral as Windows Service...');
                ...
                require('child_process').exec('node winservice.js --install', { maxBuffer: 512000, timeout: 120000, cwd: servicepath }, function (error, stdout, stderr) {
                    if ((error != null) && (error != '')) { console.log('ERROR: Unable to install RenovarCentral as a service: ' + error); process.exit(); return; }
                    console.log(stdout);
                });
                return;
            } else if (obj.args.uninstall == true) {
                console.log('Uninstalling RenovarCentral Windows Service...');
                if (obj.fs.existsSync(servicepath) == true) {
                    require('child_process').exec('node winservice.js --uninstall', { maxBuffer: 512000, timeout: 120000, cwd: servicepath }, function (error, stdout, stderr) {
                        if ((error != null) && (error != '')) { console.log('ERROR: Unable to uninstall RenovarCentral service: ' + error); process.exit(); return; }
                        console.log(stdout);
    ...
    // Launch RenovarCentral as a child server and monitor it.
        ...
        childProcess = child_process.execFile(process.argv[0], startArgs, { maxBuffer: Infinity, cwd: obj.parentpath }, function (error, stdout, stderr) {
            if (childProcess.xrestart == 1) {
                ...
                if (error != null) {
                    ...
                    console.log(error);
                    console.log('ERROR: RenovarCentral failed with critical error, check mesherrors.txt. Restarting in 5 seconds...');
    ...
    // Get current and latest RenovarCentral server versions using NPM
    ...
    // Get current version and all RenovarCentral server tags using NPM
    ...
    obj.StartEx = function () {
        ...
        if ((require('os').platform() != 'win32') && ((config.settings.syslog != null) || (config.settings.syslogjson != null) || (config.settings.syslogauth != null))) {
            ...
            if (typeof config.settings.syslog == 'string') {
                ...
                obj.syslog.log(obj.syslog.LOG_INFO, "RenovarCentral v" + getCurrentVersion() + " Server Start");
            }
            if (typeof config.settings.syslogjson == 'string') {
                ...
                obj.syslogjson.log(obj.syslogjson.LOG_INFO, "RenovarCentral v" + getCurrentVersion() + " Server Start");
            }
            if (typeof config.settings.syslogauth == 'string') {
                ...
                obj.syslogauth.log(obj.syslogauth.LOG_INFO, "RenovarCentral v" + getCurrentVersion() + " Server Start");
            }
        }
        ...
        if (config.settings.syslogtcp != null) {
            ...
            obj.syslogtcp.log("RenovarCentral v" + getCurrentVersion() + " Server Start", obj.syslogtcp.LOG_INFO);
        }
                ...
                obj.db.SetupDatabase(function (dbversion) {
                    ...
                    if (obj.args.createaccount) { // Create a new user account
                        ...
                        obj.db.Get(userid, function (err, docs) {
                            ...
                            if (obj.args.hashpass) {
                                ...
                                obj.db.Set(user, function () { console.log("Done. This command will only work if RenovarCentral is stopped."); process.exit(); return; });
                            } else {
                                // Hash the password and create the account.
                                require('./pass.js').hash(obj.args.pass, function (err, salt, hash, tag) { if (err) { console.log("Unable create account password: " + err); process.exit(); return; } user.salt = salt; user.hash = hash; obj.db.Set(user, function () { console.log("Done."); process.exit(); return; }); }, 0);
                            }
                        });
                        return;
                    }
                    if (obj.args.resetaccount) { // Unlock a user account, set a new password and remove 2FA
                        ...
                        obj.db.Get(userid, function (err, docs) {
                            ...
                            if (obj.args.hashpass) {
                                ...
                                require('./pass.js').hash(String(obj.args.pass), user.salt, function (err, hash, tag) { if (err) { console.log("Unable to reset password: " + err); process.exit(); return; } user.hash = hash; obj.db.Set(user, function () { console.log("Done."); process.exit(); return; }); }, 0);
                            }
                        });
                        return;
                    }
                    if (obj.args.adminaccount) { // Set a user account to server administrator
                        ...
                        obj.db.Get(userid, function (err, docs) {
                            ...
                            obj.db.Set(docs[0], function () { console.log("Done. This command will only work if RenovarCentral is stopped."); process.exit(); return; });
                        });
                        return;
                    }
    ...
    obj.StartEx1b = async function () {
        ...
        if (Number(process.version.match(/^v(\d+\.\d+)/)[1]) < 16) { addServerWarning("RenovarCentral will require Node v16 or above in the future, your current version is " + process.version + "."); }
        ...
        obj.db.Get('dbconfig', function (err, dbconfig) {
            ...
            if ((obj.config) && (obj.config.settings) && (obj.config.settings.taskmanager == true)) {
                obj.taskManager = require('./taskmanager.js').createTaskManager(obj);
            }
    ...
    obj.StartEx4 = function () {
        ...
        console.log("RenovarCentral v" + getCurrentVersion() + ', ' + (["Hybrid (LAN + WAN) mode", "WAN mode", "LAN mode"][runmode]) + (productionMode ? ", Production mode." : '.'));
        ...
        // Load the list of RenovarCentral tools
        obj.updateMeshTools();
            ...
            obj.updateMeshAgentsTable(obj.config.domains[''], function () {
                ...
                obj.crypto.randomBytes(48, function (err, buf) {
                    ...
                    if ((config.firebase != null) && (typeof config.firebase.senderid == 'string') && (typeof config.firebase.serverkey == 'string')) {
                        obj.firebase = require('./firebase.js').CreateFirebase(obj, config.firebase.senderid, config.firebase.serverkey);
                    } else if ((typeof config.firebaserelay == 'object') && (typeof config.firebaserelay.url == 'string')) {
                        // Setup the push messaging relay
                        obj.firebase = require('./firebase.js').CreateFirebaseRelay(obj, config.firebaserelay.url, config.firebaserelay.key);
                        ...
                        obj.firebase = require('./firebase.js').CreateFirebaseRelay(obj, 'https://meshcentral.com/firebaserelay.aspx');
                    }
                    ...
                    if ((typeof config.settings.amtprovisioningserver == 'object') && (typeof config.settings.amtprovisioningserver.devicegroup == 'string') && (typeof config.settings.amtprovisioningserver.newmebxpassword == 'string') && (typeof config.settings.amtprovisioningserver.trustedfqdn == 'string') && (typeof config.settings.amtprovisioningserver.ip == 'string')) {
                        obj.amtProvisioningServer = require('./amtprovisioningserver.js').CreateAmtProvisioningServer(obj, config.settings.amtprovisioningserver);
                    }
    ...
    obj.StartEx5 = function () {
        ...
        if (ipKvmSupport) { obj.ipKvmManager = require('./meshipkvm.js').CreateIPKVMManager(obj); }
    ...
    const meshToolsList = {
        'MeshCentralRouter': { localname: 'RenovarRouter.exe', dlname: 'winrouter' },
        'MeshCentralAssistant': { localname: 'MeshCentralAssistant.exe', dlname: 'winassistant', winhash: true }
        //'MeshCentralRouterMacOS': { localname: 'RenovarRouter.dmg', dlname: 'RenovarRouter.dmg' }
    };
        ...
        for (var toolname in meshToolsList) {
            if (meshToolsList[toolname].winhash === true) {
                ...
                hashStream.on('data', function (data) {
                    ...
                    obj.meshToolsBinaries[this.toolname].url = 'https://' + obj.certificates.CommonName + ':' + ((typeof obj.args.aliasport == 'number') ? obj.args.aliasport : obj.args.port) + '/renovar?meshaction=' + this.dlname;
                    var stats = null;
        ...
        for (var scriptid in meshAgentsInstallScriptList) {
            ...
            try {
                ...
                stream.on('end', function () {
                    ...
                    obj.meshAgentInstallScripts[this.info.id].url = 'https://' + obj.certificates.CommonName + ':' + ((typeof obj.args.aliasport == 'number') ? obj.args.aliasport : obj.args.port) + '/renovar?script=' + this.info.id;
                    var stats = null;
                    try { stats = obj.fs.statSync(this.agentpath); } catch (ex) { }
    ...
    obj.meshAgentsArchitectureNumbers = {
        ...
        34: { id: 34, localname: 'assistant_windows', rname: 'meshassistant', desc: 'RenovarCentral Assistant (Windows)', update: false, amt: false, platform: 'assistant', core: 'linux-noamt', rcore: 'linux-recovery', arcore: 'linux-agentrecovery', tcore: 'linux-tiny' }, // RenovarCentral Assistant for Windows
        ...
        10006: { id: 10006, localname: 'MeshCentralAssistant.exe', rname: 'RenovarAssistant.exe', desc: 'RenovarCentral Assistant for Windows', update: false, amt: false, platform: 'win32' }, // RenovarCentral Assistant
        11000: { id: 11000, localname: 'MeshCmd.exe', rname: 'RenoCmd.exe', desc: 'Windows x86-32 meshcmd', update: false, amt: true, platform: 'win32', codesign: true }, // MeshCMD for Windows x86 32-bit
        11001: { id: 11001, localname: 'MeshCmd64.exe', rname: 'RenoCmd64.exe', desc: 'Windows x86-64 meshcmd', update: false, amt: true, platform: 'win32', codesign: true }, // MeshCMD for Windows x86 64-bit
        11002: { id: 11002, localname: 'MeshCmdARM64.exe', rname: 'RenoCmdARM64.exe', desc: 'Windows ARM-64 meshcmd', update: false, amt: true, platform: 'win32', codesign: true } // MeshCMD for Windows ARM 64-bit
    };
    ...
    obj.updateMeshAgentsTable = function (domain, func) {
        ...
        for (var archid in obj.meshAgentsArchitectureNumbers) {
            ...
            objx.meshAgentBinaries[archid].url = 'http://' + obj.certificates.CommonName + ':' + ((typeof obj.args.aliasport == 'number') ? obj.args.aliasport : obj.args.port) + '/renovar?id=' + archid;
...
function InstallModule(modulename, func, tag1, tag2) {
    ...
    child_process.exec(npmpath + ` install --no-audit --no-package-lock --no-optional --save-exact ${modulename}`, { maxBuffer: 512000, timeout: 120000, cwd: parentpath }, function (error, stdout, stderr) {
        if ((error != null) && (error != '')) {
            var mcpath = __dirname;
            if (mcpath.endsWith('\\node_modules\\meshcentral') || mcpath.endsWith('/node_modules/meshcentral')) { mcpath = require('path').join(mcpath, '..', '..'); }
            console.log('ERROR: Unable to install required module "' + modulename + '". RenovarCentral may not have access to npm, or npm may not have suffisent rights to load the new module. To manualy install this module try:\r\n\r\n   cd "' + mcpath + '"\r\n   npm install ' + modulename + '\r\n   node node_modules' + ((require('os').platform() == 'win32') ? '\\' : '/') + 'meshcentral');
            process.exit();
            return;
        }
...
/*
var ServerWarnings = {
    1: "RenovarCentral SSH support requires NodeJS 11 or higher.",
...
function mainStart() {
    ...
    if (Number(process.version.match(/^v(\d+\.\d+)/)[1]) < 10) { console.log("RenovarCentral requires Node v10 or above, current version is " + process.version + "."); return; }
    ...
    InstallModules(['minimist'], function () {
        ...
        var mstsc = true;
        try { require('./rdp/index.js') } catch (ex) { mstsc = false; }
        ...
        if (ssh == true) { if (nodeVersion < 11) { addServerWarning('RenovarCentral SSH support requires NodeJS 11 or higher.', 1); } else { modules.push('ssh2'); } }
        ...
        InstallModules(modules, function () {
            if (require('os').platform() == 'win32') { try { require('node-windows'); } catch (ex) { console.log("Module node-windows can't be loaded. Restart RenovarCentral."); process.exit(); return; } }
