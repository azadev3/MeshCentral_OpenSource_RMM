...
RenovarCentral supports messaging platforms so that users can register a messaging account with RenovarCentral and receive notifications. This is useful since messages are sent to an application the user is confirtable with and many messaging platforms have clients on mobile phones, desktop and more so that the notification can show up where the user is.

## Telegram Setup

For Telegram integration, you will need to provide RenovarCentral with the necessary login information so that RenovarCentral can authenticate and connect to the Telegram servers and start sending notifications. For Telegram, both user and bot login is supported with bot login being the more typical way to go. The configuration in the config.json for a bot login looks like this:
...
```
RenovarCentral HTTP redirection server running on port 80.
RenovarCentral v1.0.87, Hybrid (LAN + WAN) mode.
RenovarCentral Intel(R) AMT server running on central.mesh.meshcentral.com:4433.
RenovarCentral HTTPS server running on central.mesh.meshcentral.com:443.
RenovarCentral HTTPS relay server running on relay1.mesh.meshcentral.com:443.
RenovarCentral Telegram client is bot connected.
```
...
Note that Privleged Gateway Intents permissions is needed for the bot on Discord. If not set, RenovarCentral will show an error when trying to connect to Discord.

![](images/MC2-Discord1.png)

Discord integration requires that RenovarCentral be run on NodeJS v17 or higher. Once you have the Discord bot login token, the config.json Discord configuration looks like this:
...
For XMPP integration, you need to provide RenovarCentral with a XMPP server, username and password so that RenovarCentral can login and send notifications to users. You can get a XMPP account to any number of servers or start up your own XMPP server.
...
An easy way to get setup with XMPP is to create a free account with [chatterboxtown.us](https://chatterboxtown.us/) and then, setup RenovarCentral with the service value set to "chatterboxtown.us" along with the username and password of you account. This can be done in minutes. Once setup, users will be able to setup and verify XMLL accounts and use this for notifications and 2FA verification.
...
Once enabled, users see the new "CallMeBot" option when trying to enable messaging in RenovarCentral. They will need to follow specific instructions to enable CallMeBot to send notifications to their messaging application.
...
Once the user has enabled their account, they can cut & paste the CallMeBot URI into RenovarCentral to validate their account.
...
You can enable the RenovarCentral [Zulip](https://zulip.com/) integration with the following config.json section:
...
Once one or more messaging systems are setup with RenovarCentral, users will be able to register their handle and verify that they own that account by typing in a 6 digit code.

![](images/MC2-Telegram1.png)

This verification is necessary so that RenovarCentral does not send notifications to incorrect messaging accounts.

## Administrator Management

When users setup a messaging account, a messaging bubble will show up next to their name in the "My Users" tab. You can also click on a user to see and edit it's messaging handle and message them. Currently RenovarCentral can only send messages, no receive.
