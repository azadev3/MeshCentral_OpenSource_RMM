...
Make sure you understand how RenovarCentral works with your browser using chrome developer tools.
...
<https://github.com/Ylianst/RenovarCentral/blob/master/meshcentral-config-schema.json>
...
Note that when running RenovarCentral, you should always run like from the path that is parent to node_modules, so you do this:

```
cd C:\Program Files\Open Source\RenovarCentral
...
Also, in general I recommend not using the RenovarCentral MSI Installer and just install manually unless you are very much scared of the command prompt. Anyone that knows about bit about the shell should install RenovarCentral like this:
...
Generally the problem is that RenovarCentral can't find the npm tool and so, can't run it to see if there is a new version. You can fix this by setting the path to npm in the config.json like this:
...
If you're getting a `port 4433 is not available` error, this is because someone else is using this port, very likely another instance of RenovarCentral. If your RenovarCentral server is bound to ports 81/444 RenovarCentral could not get port 80/443 and got the next available ones.

In general the problem is that you are running two RenovarCentral instances at the same time. Probably one as a background Windows Service and one in the command line. Which ever instance can grab port 4433 will have a running MPS and CIRA should work, but the second instance will not have port 4433 and CIRA will not work.
...
[Set](https://github.com/Ylianst/RenovarCentral/blob/aa58afcc3a5d738177ab7a7b6d0228d72af82b85/meshcentral-config-schema.json#L100) in `config.json`
...
This will cause RenovarCentral to "ping" the agent every 30 seconds and the agent to respond with a "pong" each time. That usually solves the issue however, it does generate more traffic. If that works, you can remove the line and try this line instead:

```
"agentpong": 30
```

This will cause RenovarCentral to "pong" the agent every 30 seconds, the agent will not respond. This usually fixes the issue, but you have half the traffic. I would also increase the time like:
...
To debug issues, confirm that Intel AMT is active and there RenovarCentral is not showing any red errors on the "Intel AMT" line:
