/**
* @description RenovarCentral MeshAgent communication module
...
module.exports.CreateMeshAgent = function (parent, db, ws, req, args, domain) {
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if ((nodes == null) || (nodes.length == 0)) {
                ...
                if (change == 1) {
                    ...
                    var event = {correlationid: event?.correlationid || obj?.correlationid || command?.correlationid || domain?.correlationid || parent?.correlationid, etype: 'node', action: 'changenode', nodeid: obj.dbNodeKey, domain: domain.id, node: parent.CloneSafeNode(device) };
    ...
    function processAgentData(msg) {
        ...
        if (str[0] == '{') {
            ...
            switch (command.action) {
                ...
                case 'meshToolInfo': {
                    // Return information about a RenovarCentral tool. Current tools are 'MeshCentralRouter' and 'MeshCentralAssistant'
                    ...
                    if ((command.name == 'MeshCentralAssistant') && (command.msh == true)) { responseCmd.url = '*/' + xdomain + 'renovar?id=10006'; } // If this is Assistant and the MSH needs to be included in the executable, change the URL.
                ...
                case 'getServerImage': {
                    if (command.agent === 'assistant') {
                        // Return server title and image for RenovarCentral Assistant
    ...
    obj.updateSessions = function () {
        ...
        var event = {correlationid: event?.correlationid || obj.correlationid || domain?.correlationid, etype: 'node', action: 'devicesessions', nodeid: obj.dbNodeKey, domain: domain.id, sessions: obj.sessions, nolog: 1 };
        parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(obj.dbMeshKey, [obj.dbNodeKey]), obj, event);
    }
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if (device.agent) {
                ...
                if (change == 1) {
                    ...
                    // Event the node change
                    var event = { correlationid: event?.correlationid || obj?.correlationid || command?.correlationid || domain?.correlationid || parent?.correlationid, etype: 'node', action: 'changenode', nodeid: obj.dbNodeKey, domain: domain.id, node: parent.CloneSafeNode(device), msgid: 59, msgArgs: [device.name, mesh.name, changes.join(', ')], msg: 'Changed device ' + device.name + ' from group ' + mesh.name + ': ' + changes.join(', ') };
        ...
        db.Get(obj.dbNodeKey, function (err, nodes) {
            ...
            if (device.agent) {
                ...
                if (changes == true) {
                    ...
                    // Event the node change
                    var event = { correlationid: event?.correlationid || obj?.correlationid || command?.correlationid || domain?.correlationid || parent?.correlationid, etype: 'node', action: 'changenode', nodeid: obj.dbNodeKey, domain: domain.id, node: parent.CloneSafeNode(device), nolog: 1 };
                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the node. Another event will come.
                    parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(device.meshid, [obj.dbNodeKey]), obj, event);
                }
            }
