/**
* @description RenovarCentral user messaging communication module
...
module.exports.CreateServer = function (parent) {
        ...
        if (telegramOK) {
            ...
            async function setupTelegram() {
                ...
                if (parent.config.messaging.telegram.bottoken == null) {
                    ...
                    console.log("RenovarCentral Telegram client is user connected.");
                } else {
                    // Bot login
                    ...
                    obj.telegramClient = client;
                    obj.providers += 1; // Enable Telegram messaging
                    console.log("RenovarCentral Telegram client is bot connected.");
                }
            }
            setupTelegram();
        }
    ...
    if (parent.config.messaging.discord) {
        ...
        if (discordOK) {
            ...
            discordClient.on('ready', function() {
                console.log(`RenovarCentral Discord client is connected as ${discordClient.user.tag}!`);
    ...
    if (parent.config.messaging.xmpp) {
        ...
        if (xmppOK) {
            ...
            xmpp.on('online', async function (address) {
                ...
                console.log("RenovarCentral XMPP client is connected.");
            });
    ...
    obj.sendMessage = function(to, msg, domain, func) {
        if ((to.startsWith('telegram:')) && (obj.telegramClient != null)) { // Telegram
            ...
            push.send(domain.title ? domain.title : 'RenovarCentral', msg, function (err, res) { if (func != null) { func(err == null); } });
            ...
            obj.zulipClient.sendMessage({
                ...
                content: msg,
                to: [ to.substring(6) ],
                subject: domain.title ? domain.title : 'RenovarCentral'
            });
            if (func != null) { func(true); }
        } else {
            // No providers found
    ...
    obj.sendMessagingCheck = function (domain, to, verificationCode, language, func) {
        ...
        // Setup the template
        sms = sms.split('[[0]]').join(domain.title ? domain.title : 'RenovarCentral');
