/**
* @description RenovarCentral MeshAgent
...
module.exports.CreateMeshUser = function (parent, db, ws, req, args, domain, user) {
    ...
    // RenovarCentral Satellite
    const SATELLITE_PRESENT = 1;     // This session is a RenovarCentral Salellite session
    ...
    */
    ...
    try {
        ...
        parent.crypto.randomBytes(20, function (err, randombuf) {
                ...
                if ((event.domain == null) || (event.domain == domain.id) || ((obj.crossDomain === true) && (allowedCrossDomainMessages.indexOf(event.action) >= 0))) {
                    try {
                        if (event == 'close') { try { delete req.session; } catch (ex) { } obj.close(); return; }
                            ...
                            // This is a RenovarCentral Satellite message
                            ...
                            if (id.startsWith('mesh/')) {
                                ...
                                if ((meshrights === MESHRIGHT_ADMIN) || ((meshrights & MESHRIGHT_LIMITEVENTS) == 0) || (ids.indexOf(user._id) >= 0)) {
                                    ...
                                    try { ws.send(JSON.stringify({correlationid: event?.correlationid, action: 'event', event: event })); } catch (ex) { }
                                } else {
                                    ...
                                    if (userTarget == false) { ws.send(JSON.stringify({correlationid: event?.correlationid, action: 'event', event: event })); }
                                }
                                ...
                                if ((user.siteadmin & SITERIGHT_USERGROUPS) != 0) {
                                    ...
                                    try { ws.send(JSON.stringify({correlationid: event?.correlationid, action: 'event', event: { ugrpid: event.ugrpid, domain: event.domain, time: event.time, name: event.name, action: event.action, username: event.username, links: links, h: event.h } })); } catch (ex) { }
                                }
                            } else {
            ...
            obj.SendServerStats = function () {
                ...
                var os = require('os');
                var stats = {correlationid: domain?.correlationid, action: 'serverstats', totalmem: os.totalmem(), freemem: os.freemem() };
            ...
            try { ws.send(JSON.stringify({correlationid: domain?.correlationid, action: 'serverinfo', serverinfo: serverinfo })); } catch (ex) { }

            // Send user information to web socket, this is the first thing we send
            try { ws.send(JSON.stringify({correlationid: domain?.correlationid, action: 'userinfo', userinfo: parent.CloneSafeUser(parent.users[user._id]) })); } catch (ex) { }
            ...
            if (user.siteadmin === SITERIGHT_ADMIN) {
                ...
                if ((domain.myserver !== false) && ((domain.myserver == null) || (domain.myserver.trace === true))) {
                    ...
                    try { ws.send(JSON.stringify({correlationid: parent?.correlationid || domain?.correlationid, action: 'traceinfo', traceSources: parent.parent.debugRemoteSources })); } catch (ex) { }
                }
            ...
            if (lastLoginTime != null) {
                db.GetFailedLoginCount(user._id, user.domain, new Date(lastLoginTime * 1000), function (count) {
                    if (count > 0) { try { ws.send(JSON.stringify({correlationid: common?.correlationid || parent?.correlationid || command?.correlationid, action: 'msg', type: 'notify', title: "Security Warning", tag: 'ServerNotify', id: Math.random(), value: "There has been " + count + " failed login attempts on this account since the last login.", titleid: 3, msgid: 12, args: [count] })); } catch (ex) { } delete user.pastlogin; }
                });
            }
            ...
            switch (command.action) {
                case 'nodes':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'nodes', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        db.GetAllTypeNoTypeFieldMeshFiltered(links, extraids, domain.id, 'node', command.id, obj.deviceSkip, obj.deviceLimit, function (err, docs) {
                            ...
                            const response = { correlationid: command?.correlationid, action: 'nodes', responseid: command.responseid, nodes: r, tag: command.tag };
                            if (domain.maxdeviceview != null) {
                ...
                case 'msg':
                    {
                        ...
                        if (common.validateString(command.nodeid, 1, 1024) == false) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid || event?.correlationid || paren?.correlationid, action: 'msg', result: 'Unable to route', tag: command.tag, responseid: command.responseid })); } catch (ex) { } }
                            return;
                        }
                        ...
                        var func = null;
                        if (command.responseid != null) { func = function (r) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', result: r ? 'OK' : 'Unable to route', tag: command.tag, responseid: command.responseid })); } catch (ex) { } } }
                        ...
                        if ((command.userid != null) && ((user.siteadmin & SITERIGHT_MANAGEUSERS) != 0)) {
                            ...
                            if ((command.limit == null) || (typeof command.limit != 'number')) {
                                ...
                                db.GetUserEvents(filter, domain.id, command.userid, function (err, docs) {
                                    if (err != null) return;
                                    try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'events', events: docs, userid: command.userid, tag: command.tag })); } catch (ex) { }
                                });
                            } else {
                            ...
                            parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                                if (node == null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'events', events: [], nodeid: command.nodeid, tag: command.tag })); } catch (ex) { } return; }
                                ...
                                if (((rights & MESHRIGHT_LIMITEVENTS) != 0) && (rights != MESHRIGHT_ADMIN)) {
                                    ...
                                    db.GetNodeEventsSelfWithLimit(node._id, domain.id, user._id, limit, function (err, docs) {
                                        if (err != null) return;
                                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'events', events: docs, nodeid: node._id, tag: command.tag })); } catch (ex) { }
                                    });
                                } else {
                            ...
                            if ((command.limit == null) || (typeof command.limit != 'number')) {
                                ...
                                db.GetEvents(filter, domain.id, function (err, docs) {
                                    if (err != null) return;
                                    try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'events', events: docs, user: command.user, tag: command.tag })); } catch (ex) { }
                                });
                            } else {
                                // Send the list of most recent events for this session, up to 'limit' count
                ...
                case 'recordings': {
                    ...
                    fs.readdir(recordingsPath, function (err, files) {
                        if (err != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'recordings', error: 1, tag: command.tag })); } catch (ex) { } return; }
                        if ((command.limit == null) || (typeof command.limit != 'number')) {
                            // Send the list of all recordings
                            db.GetEvents(['recording'], domain.id, function (err, docs) {
                                if (err != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'recordings', error: 2, tag: command.tag })); } catch (ex) { } return; }
                                for (var i in docs) {
                                ...
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'recordings', events: docs, tag: command.tag })); } catch (ex) { }
                            });
                        } else {
                ...
                case 'wssessioncount':
                    {
                        ...
                        var wssessions = {};
                        if ((user.siteadmin & 2) == 0) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wssessioncount', wssessions: {}, tag: command.tag })); } catch (ex) { } break; }
                        if (parent.parent.multiServer == null) {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wssessioncount', wssessions: wssessions, tag: command.tag })); } catch (ex) { } // wssessions is: userid --> count
                        break;
                    }
                case 'deleteuser':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deleteuser', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                            ...
                            for (var i in deluser.links) {
                                if (i.startsWith('mesh/')) {
                                    ...
                                    if (mesh) {
                                        ...
                                        var event = {correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: mesh._id, name: mesh.name, mtype: mesh.mtype, desc: mesh.desc, correlationid: command?.correlationid, action: 'meshchange', links: mesh.links, msgid: 72, msgArgs: [deluser.name, mesh.name], msg: change, domain: deluserdomain.id, invite: mesh.invite };
                                    ...
                                    parent.GetNodeWithRights(deluserdomain, deluser, i, function (node, rights, visible) {
                                        ...
                                        var event;
                                        if (command.rights == 0) {
                                            event = {correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: deluserdomain.id, msgid: 60, msgArgs: [node.name], msg: 'Removed user device rights for ' + node.name, node: parent.CloneSafeNode(node) }
                                        } else {
                                            event = {correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: deluserdomain.id, msgid: 61, msgArgs: [node.name], msg: 'Changed user device rights for ' + node.name, node: parent.CloneSafeNode(node) }
                                        }
                                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                                    ...
                                    if (ugroup) {
                                        ...
                                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: ugroup._id, name: ugroup.name, desc: ugroup.desc, correlationid: command?.correlationid, action: 'usergroupchange', links: ugroup.links, msgid: 62, msgArgs: [deluser.name, ugroup.name], msg: 'Removed user ' + deluser.name + ' from user group ' + ugroup.name, addUserDomain: deluserdomain.id };
                        ...
                        parent.parent.DispatchEvent(targets, obj, { etype: 'user', userid: deluserid, username: deluser.name, correlationid: command?.correlationid, action: 'accountremove', msgid: 63, msg: 'Account removed', domain: deluserdomain.id });
                        parent.parent.DispatchEvent([deluserid], obj, 'close');
           
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deleteuser', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                ...
                case 'userbroadcast':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'userbroadcast', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
           
                        // Create the notification message
                        var notification = { correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'notify', domain: domain.id, value: command.msg, title: user.name, icon: 0, tag: 'broadcast', id: Math.random() };
                        ...
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'userbroadcast', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'edituser':
                    {
                        ...
                        if (err != null) {
                            if (command.responseid != null) {
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'edituser', responseid: command.responseid, result: err })); } catch (ex) { }
                            }
                            break;
                        }
                        ...
                        if (chguser) {
                            ...
                            if (change == 1) {
                                ...
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(chguser), correlationid: command?.correlationid, action: 'accountchange', msgid: 66, msgArgs: [chguser.name], msg: 'Account changed: ' + chguser.name, domain: edituserdomain.id };
                        ...
                        // OK Response
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'edituser', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'usergroups':
                    {
                        ...
                        if ((user.siteadmin & SITERIGHT_USERGROUPS) == 0) {
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'usergroups', ugroups: groupCount ? groups : null, tag: command.tag })); } catch (ex) { }
                        } else {
                ...
                case 'createusergroup':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createusergroup', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        parent.crypto.randomBytes(48, function (err, buf) {
                                    ...
                                    for (var i in cgroup.links) {
                                        if (i.startsWith('user/')) {
                                            ...
                                            if ((xuser != null) && (xuser.links != null)) {
                                                ...
                                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(xuser), correlationid: command?.correlationid, action: 'accountchange', msgid: 67, msgArgs: [xuser.name], msg: 'User group membership changed: ' + xuser.name, domain: ugrpdomain.id };
                                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                                //parent.parent.DispatchEvent(targets, obj, event);
                                            ...
                                            if (xmesh && xmesh.links) {
                                                ...
                                                // Notify mesh change
                                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: xmesh._id, name: xmesh.name, mtype: xmesh.mtype, desc: xmesh.desc, correlationid: command?.correlationid, action: 'meshchange', links: xmesh.links, msgid: 68, msgArgs: [ugrp.name, xmesh.name], msg: 'Added user group ' + ugrp.name + ' to device group ' + xmesh.name, domain: ugrpdomain.id, invite: xmesh.invite };
                                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                            ...
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: ugrpid, name: ugrp.name, desc: ugrp.desc, correlationid: command?.correlationid, action: 'createusergroup', links: ugrp.links, msgid: 69, msgArgv: [ugrp.name], msg: 'User group created: ' + ugrp.name, ugrpdomain: domain.id };
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createusergroup', responseid: command.responseid, result: 'ok', ugrpid: ugrpid, links: ugrp.links })); } catch (ex) { }
                        });
                        break;
                    }
                case 'deleteusergroup':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deleteusergroup', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
           
                        db.Get(command.ugrpid, function (err, groups) {
                            if ((err != null) || (groups.length != 1)) {
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deleteusergroup', responseid: command.responseid, result: 'Unknown device group' })); } catch (ex) { }
                                return;
                            }
                            var group = groups[0];
                                ...
                                for (var i in group.links) {
                                    if (i.startsWith('user/')) {
                                        ...
                                        if ((xuser != null) && (xuser.links != null)) {
                                            ...
                                            // Notify user change
                                            var targets = ['*', 'server-users', user._id, xuser._id];
                                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(xuser), correlationid: command?.correlationid, action: 'accountchange', msgid: 67, msgArgs: [xuser.name], msg: 'User group membership changed: ' + xuser.name, delGroupDomain: domain.id };
                                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                            parent.parent.DispatchEvent(targets, obj, event);
                                        }
                                    } else if (i.startsWith('mesh/')) {
                                        ...
                                        if (xmesh && xmesh.links) {
                                            ...
                                            // Notify mesh change
                                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: xmesh._id, name: xmesh.name, mtype: xmesh.mtype, desc: xmesh.desc, correlationid: command?.correlationid, action: 'meshchange', links: xmesh.links, msgid: 70, msgArgs: [group.name, xmesh.name], msg: 'Removed user group ' + group.name + ' from device group ' + xmesh.name, domain: delGroupDomain.id };
                                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                            ...
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: group._id, correlationid: command?.correlationid, action: 'deleteusergroup', msg: change, domain: delGroupDomain.id };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deleteusergroup', responseid: command.responseid, result: 'ok', ugrpid: group._id })); } catch (ex) { }
                        });
                        break;
                    }
                case 'editusergroup':
                    {
                        ...
                        if (group != null) {
                            ...
                            if (change != '') {
                                db.Set(group);
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: group._id, name: group.name, desc: group.desc, consent: ((group.consent == null) ? 0 : group.consent), correlationid: command?.correlationid, action: 'usergroupchange', links: group.links, flags: group.flags, msg: change, domain: domain.id };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                                parent.parent.DispatchEvent(['*', group._id, user._id], obj, event);
                            }
                        }
                ...
                case 'changemeshnotify':
                    {
                        ...
                        // Handle any errors
                        if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changemeshnotify', responseid: command.responseid, result: err })); } catch (ex) { } } break; }
                        ...
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 73, msg: 'Device group notification changed', domain: domain.id };
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                ...
                case 'changeusernotify':
                    {
                        ...
                        // Handle any errors
                        if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeusernotify', responseid: command.responseid, result: err })); } catch (ex) { } } break; }
                        ...
                        var targets = ['*', 'server-users', user._id];
                        if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 130, msg: 'User notifications changed', domain: domain.id };
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                                    ...
                                    if (result == 1) {
                                        ...
                                        require('./pass').hash(command.newpass, function (err, salt, hash, tag) {
                                            if (err) {
                                                ...
                                                var targets = ['*', 'server-users'];
                                                if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 74, msgArgs: [user.name], msg: 'Account password changed: ' + user.name, domain: domain.id };
                                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                                parent.parent.DispatchEvent(targets, obj, event);
                        ...
                        if (chguser) {
                            ...
                            require('./pass').hash(command.pass, function (err, salt, hash, tag) {
                                if (!err) {
                                    ...
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(chguser), correlationid: command?.correlationid, action: 'accountchange', msgid: 75, msg: 'Changed account credentials', domain: domain.id };
                                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                    parent.parent.DispatchEvent(targets, obj, event);
                ...
                case 'notifyuser':
                    {
                        ...
                        // Create the notification message
                        var notification = { correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'notify', id: Math.random(), value: command.msg, title: user.name, icon: 8, userid: user._id, username: user.name };
                        ...
                        if ((parent.wssessions[chguser._id] == null) && (parent.sessionsCount[chguser._id] == null)) {
                            ...
                            if (domain.title != null) { payload.title = domain.title; } else { payload.title = "RenovarCentral"; }
                ...
                case 'createmesh':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createmesh', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        parent.crypto.randomBytes(48, function (err, buf) {
                            ...
                            var targets = ['*', 'server-users', user._id];
                            if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', domain: domain.id, nolog: 1 };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                            parent.parent.DispatchEvent(targets, obj, event);
           
                            // Event the device group creation
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: meshid, mtype: command.meshtype, mesh: parent.CloneSafeMesh(mesh), correlationid: command?.correlationid, action: 'createmesh', msgid: 76, msgArgs: [command.meshname], msg: 'Device group created: ' + command.meshname, domain: domain.id };
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createmesh', responseid: command.responseid, result: 'ok', meshid: meshid, links: links })); } catch (ex) { }
                            ...
                            if (mesh.relayid != null) {
                                ...
                                parent.GetNodeWithRights(domain, user, mesh.relayid, function (node, rights, visible) {
                                    if (node == null) return;
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: domain.id, msg: 'Is a relay for ' + mesh.name + '.', msgid: 153, msgArgs: [mesh.name], node: parent.CloneSafeNode(node) };
                ...
                case 'deletemesh':
                    {
                        ...
                        // Handle any errors
                        if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deletemesh', responseid: command.responseid, result: err })); } catch (ex) { } } break; }
           
                        // Get the device group reference we are going to delete
                        var mesh = parent.meshes[command.meshid];
                        if (mesh == null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deletemesh', responseid: command.responseid, result: 'Unknown device group' })); } catch (ex) { } } return; }
                        ...
                        // Handle any errors
                        if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deletemesh', responseid: command.responseid, result: err })); } catch (ex) { } } return; }
           
                        // Fire the removal event first, because after this, the event will not route
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, mtype: mesh.mtype, meshid: command.meshid, name: command.meshname, correlationid: command?.correlationid, action: 'deletemesh', msgid: 77, msgArgs: [command.meshname], msg: 'Device group deleted: ' + command.meshname, domain: domain.id };
                        ...
                        for (var j in mesh.links) {
                            if (j.startsWith('user/')) {
                                ...
                                if (xuser && xuser.links) {
                                    ...
                                    // Notify user change
                                    var targets = ['*', 'server-users', user._id, xuser._id];
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(xuser), correlationid: command?.correlationid, action: 'accountchange', msgid: 78, msgArgs: [xuser.name], msg: 'Device group membership changed: ' + xuser.name, domain: domain.id };
                                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                    parent.parent.DispatchEvent(targets, obj, event);
                                }
                                ...
                                if (xgroup && xgroup.links) {
                                    ...
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: xgroup._id, name: xgroup.name, desc: xgroup.desc, correlationid: command?.correlationid, action: 'usergroupchange', links: xgroup.links, msgid: 79, msgArgs: [xgroup.name], msg: 'User group changed: ' + xgroup.name, domain: domain.id };
                                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                    parent.parent.DispatchEvent(targets, obj, event);
                                }
                            }
                        ...
                        if (mesh.relayid != null) {
                            ...
                            parent.GetNodeWithRights(domain, user, mesh.relayid, function (node, rights, visible) {
                                if (node == null) return;
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: domain.id, msg: 'No longer a relay for ' + mesh.name + '.', msgid: 152, msgArgs: [mesh.name], node: parent.CloneSafeNode(node) };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the node. Another event will come.
                                parent.parent.DispatchEvent(parent.CreateNodeDispatchTargets(node.meshid, node._id, [user._id]), obj, event);
                            });
                        }
                        ...
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deletemesh', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'editmesh':
                    {
                        ...
                        // Handle any errors
                        if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'editmesh', responseid: command.responseid, result: err })); } catch (ex) { } } break; }
                        ...
                        if (change != '') {
                            db.Set(mesh);
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: mesh._id, name: mesh.name, mtype: mesh.mtype, desc: mesh.desc, flags: mesh.flags, consent: mesh.consent, correlationid: command?.correlationid, action: 'meshchange', links: mesh.links, msgid: 142, msgArgs: [mesh.name, changesids], msg: change, domain: domain.id, invite: mesh.invite, expireDevs: command.expireDevs, relayid: mesh.relayid };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                        ...
                        if (newRelayNodeId != null) {
                            ...
                            parent.GetNodeWithRights(domain, user, mesh.relayid, function (node, rights, visible) {
                                if (node == null) return;
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: domain.id, node: parent.CloneSafeNode(node), nolog: 1 };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the node. Another event will come.
                                parent.parent.DispatchEvent(parent.CreateNodeDispatchTargets(node.meshid, node._id, [user._id]), obj, event);
                            });
                        }
           
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'editmesh', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'removemeshuser':
                    {
                        ...
                        if (err != null) {
                            console.log(err);
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removemeshuser', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        if (deluser != null) {
                            ...
                            if (deluser.links != null && deluser.links[command.meshid] != null) {
                                ...
                                if (deluserid.startsWith('user/')) {
                                    ...
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(deluser), correlationid: command?.correlationid, action: 'accountchange', msgid: 78, msgArgs: [deluser.name], msg: 'Device group membership changed: ' + deluser.name, domain: xdomain.id };
                                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                    parent.parent.DispatchEvent(targets, obj, event);
                                } else if (deluserid.startsWith('ugrp/')) {
                                    // Notify user group change
                                    var targets = ['*', 'server-ugroups', user._id, deluser._id];
                                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', username: user.name, ugrpid: deluser._id, name: deluser.name, desc: deluser.desc, correlationid: command?.correlationid, action: 'usergroupchange', links: deluser.links, msgid: 79, msgArgs: [deluser.name], msg: 'User group changed: ' + deluser.name, domain: xdomain.id };
                                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                    parent.parent.DispatchEvent(targets, obj, event);
                                }
                            }
                        ...
                        if (mesh.links[command.userid] != null) {
                            ...
                            var event;
                            if (deluser != null) {
                                event = { etype: 'mesh', username: user.name, userid: deluser.name, meshid: mesh._id, name: mesh.name, mtype: mesh.mtype, desc: mesh.desc, correlationid: command?.correlationid, action: 'meshchange', links: mesh.links, msgid: 83, msgArgs: [deluser.name, mesh.name], msg: 'Removed user ' + deluser.name + ' from device group ' + mesh.name, domain: xdomain.id, invite: mesh.invite };
                            } else {
                                event = { etype: 'mesh', username: user.name, userid: (deluserid.split('/')[2]), meshid: mesh._id, name: mesh.name, mtype: mesh.mtype, desc: mesh.desc, correlationid: command?.correlationid, action: 'meshchange', links: mesh.links, msgid: 83, msgArgs: [(deluserid.split('/')[2]), mesh.name], msg: 'Removed user ' + (deluserid.split('/')[2]) + ' from device group ' + mesh.name, domain: xdomain.id, invite: mesh.invite };
                            }
                            parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(mesh, [user._id, command.userid]), obj, event);
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removemeshuser', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        } else {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removemeshuser', responseid: command.responseid, result: 'User not in group' })); } catch (ex) { } }
                        }
                        break;
                    }
                case 'meshamtpolicy':
                    {
                        ...
                        if (mesh) {
                            ...
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'mesh', userid: user._id, username: user.name, meshid: mesh._id, amt: amtpolicy2, correlationid: command?.correlationid, action: 'meshchange', links: mesh.links, msgid: 141, msg: "Intel(r) AMT policy change", domain: domain.id, invite: mesh.invite };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                            ...
                            if (parent.parent.multiServer != null) { parent.parent.multiServer.DispatchMessage({ correlationid: command?.correlationid, action: 'newIntelAmtPolicy', meshid: command.meshid, amtpolicy: amtpolicy }); }
                ...
                case 'scanamtdevice':
                    {
                        ...
                        if (parent.parent.amtScanner) {
                            if (parent.parent.amtScanner.performRangeScan(user._id, command.range) == false) {
                                parent.parent.DispatchEvent(['*', user._id], obj, { correlationid: command?.correlationid, action: 'scanamtdevice', range: command.range, results: null, nolog: 1 });
                            }
                        }
                        break;
                    }
                case 'changeDeviceMesh':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        for (var i = 0; i < command.nodeids.length; i++) {
                            ...
                            parent.GetNodeWithRights(domain, user, xnodeid, function (node, rights, visible) {
                                ...
                                if (node == null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: 'Device not found' })); } catch (ex) { } } changeDeviceShareMeshIdNodeCount--; return; }
           
                                // Check if already in the right mesh
                                if (node.meshid == command.meshid) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: 'Device already in correct group' })); } catch (ex) { } } changeDeviceShareMeshIdNodeCount--; return; }
                                ...
                                try { if (parent.meshes[node.meshid].mtype != parent.meshes[command.meshid].mtype) { changeDeviceShareMeshIdNodeCount--; return; } } catch (e) {
                                    if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: 'Device groups are of different types' })); } catch (ex) { } }
                                ...
                                if (((rights & MESHRIGHT_EDITMESH) == 0) || ((targetMeshRights & MESHRIGHT_EDITMESH) == 0)) {
                                    if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: 'Permission denied' })); } catch (ex) { } }
                                    changeDeviceShareMeshIdNodeCount--;
                                    return;
                                }
                                ...
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'nodemeshchange', nodeid: node._id, node: node, oldMeshId: oldMeshId, newMeshId: command.meshid, msgid: 85, msgArgs: [node.name, newMesh.name], msg: 'Moved device ' + node.name + ' to group ' + newMesh.name, domain: domain.id };
                                ...
                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changeDeviceMesh', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                            });
                        }
                        break;
                    }
                case 'removedevices':
                    {
                        ...
                        for (i in command.nodeids) {
                            ...
                            parent.GetNodeWithRights(domain, user, command.nodeids[i], function (node, rights, visible) {
                                    ...
                                    for (var i in node.links) {
                                        if (i.startsWith('user/')) {
                                            ...
                                            if ((cuser != null) && (cuser.links != null) && (cuser.links[node._id] != null)) {
                                                ...
                                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: cuser._id, username: cuser.name, correlationid: command?.correlationid, action: 'accountchange', msgid: 86, msgArgs: [cuser.name], msg: 'Removed user device rights for ' + cuser.name, domain: domain.id, account: parent.CloneSafeUser(cuser) };
                                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                                parent.parent.DispatchEvent(targets, obj, event);
                                            }
                                        }
                                ...
                                // Event node deletion
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'removenode', nodeid: node._id, msgid: 87, msgArgs: [node.name, parent.meshes[node.meshid].name], msg: 'Removed device ' + node.name + ' from device group ' + parent.meshes[node.meshid].name, domain: domain.id };
                        ...
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removedevices', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                ...
                case 'wakedevices':
                    {
                        ...
                        parent.parent.DispatchEvent('*', obj, { correlationid: command?.correlationid, action: 'wakedevices', userid: user._id, username: user.name, nodeids: command.nodeids, domain: domain.id, nolog: 1 });
                        ...
                        for (i in command.nodeids) {
                            ...
                            if (common.validateString(nodeid, 8, 128) == false) { // Check the nodeid
                                if (command.nodeids.length == 1) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'Invalid nodeid' })); } catch (ex) { } }
                                continue;
                            }
                                ...
                                if (command.nodeids.length == 1) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'Invalid domain' })); } catch (ex) { } }
                                continue;
                            }
                            ...
                            parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
                                ...
                                if (node.mtype == 4) {
                                    ...
                                    const event = { etype: 'node', userid: user._id, username: user.name, nodeid: node._id, correlationid: command?.correlationid, action: 'turnon', domain: domain.id, portid: node.portid, porttype: node.porttype, portnum: node.portnum, meshid: node.meshid, mtype: node.mtype, msgid: 132, msg: "Turn on." };
                                    parent.parent.DispatchEvent(targets, obj, event);
                                    return;
                                }
                                ...
                                db.Get('if' + node._id, function (err, nodeifs) {
                                    if ((nodeifs != null) && (nodeifs.length == 1)) {
                                        ...
                                        if (macs.length == 0) {
                                            if (command.nodeids.length == 1) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'No known MAC addresses for this device' })); } catch (ex) { } }
                                            return;
                                        }
                                        ...
                                        for (j in parent.wsagents) {
                                            ...
                                            if ((agent.authenticated == 2) && ((targets.indexOf(agent.dbMeshKey) >= 0) || (targets.indexOf(agent.dbNodeKey) >= 0))) {
                                                ...
                                                try { agent.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakeonlan', macs: macs })); wakeCount++; } catch (ex) { }
                                            }
                                        }
                                        if (command.nodeids.length == 1) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'Used ' + wakeCount + ' device(s) to send wake packets' })); } catch (ex) { } }
                                    } else {
                                        if (command.nodeids.length == 1) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'No network information for this device' })); } catch (ex) { } }
                                    }
                                });
                            });
                            ...
                            if (command.nodeids.length > 1) {
                                ...
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'wakedevices', responseid: command.responseid, result: 'ok' })); } catch (ex) { }
                            }
                        }
                        break;
                    }
                case 'runcommands':
                    {
                        ...
                        const processRunCommand = function (command) {
                            for (i in command.nodeids) {
                                ...
                                if (err != null) {
                                    if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: err })); } catch (ex) { } }
                                    continue;
                                }
                                ...
                                parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
                                    ...
                                    if (node == null) {
                                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'Invalid nodeid' })); } catch (ex) { } }
                                        return;
                                    }
                                    ...
                                    if (command.type == 4) {
                                        ...
                                        if ((rights & 24) != 24) {
                                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'Access denied' })); } catch (ex) { } }
                                            return;
                                        }
                                        ...
                                        if ((agent != null) && (agent.authenticated == 2) && (agent.agentInfo != null)) {
                                            try { agent.send(JSON.stringify({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'console', value: command.cmds, rights: rights, sessionid: ws.sessionId })); } catch (ex) { }
                                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'OK' })); } catch (ex) { } }
                                        } else {
                                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'Agent not connected' })); } catch (ex) { } }
                                        }
                                    } else {
                                        ...
                                        if ((agent != null) && (agent.authenticated == 2) && (agent.agentInfo != null)) {
                                            ...
                                            if (commandsOk == true) {
                                                // Send the commands to the agent
                                                try { agent.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', type: command.type, cmds: command.cmds, runAsUser: command.runAsUser })); } catch (ex) { }
                                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'OK' })); } catch (ex) { } }
                                                ...
                                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, nodeid: node._id, correlationid: command?.correlationid, action: 'runcommands', msg: 'Running commands', msgid: msgid, cmds: command.cmds, cmdType: command.type, runAsUser: command.runAsUser, domain: domain.id };
                                                parent.parent.DispatchEvent(targets, obj, event);
                                            } else {
                                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'Invalid command type' })); } catch (ex) { } }
                                            }
                                        } else {
                                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'runcommands', responseid: command.responseid, result: 'Agent not connected' })); } catch (ex) { } }
                                        }
                        ...
                        for (i in command.nodeids) {
                            ...
                            parent.GetNodeWithRights(domain, user, command.nodeids[i], function (node, rights, visible) {
                                ...
                                if (agent != null) {
                                    ...
                                    try { agent.send(JSON.stringify({ correlationid: command?.correlationid, action: 'uninstallagent' })); } catch (ex) { }
                                }
                            });
                        }
                        break;
                        ...
                        for (i in command.nodeids) {
                            ...
                            parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
                                if ((command.actiontype >= 400) && ((rights & MESHRIGHT_REMOTECONTROL) != 0)) {
                                    // Flash and vibrate
                                    if ((command.actiontype == 400) && common.validateInt(command.time, 1, 30000)) { routeCommandToNode({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'console', nodeid: node._id, value: 'flash ' + command.time }, MESHRIGHT_ADMIN, 0); }
                                    if ((command.actiontype == 401) && common.validateInt(command.time, 1, 30000)) { routeCommandToNode({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'console', nodeid: node._id, value: 'vibrate ' + command.time }, MESHRIGHT_ADMIN, 0); }
                                } else {
                                    ...
                                    if (node.mtype == 4) {
                                        // Send out an event to perform turn off command on the port
                                        const targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, ['devport-operation', 'server-users', user._id]);
                                        const event = { etype: 'node', userid: user._id, username: user.name, nodeid: node._id, correlationid: command?.correlationid, action: 'turnoff', domain: domain.id, portid: node.portid, porttype: node.porttype, portnum: node.portnum, meshid: node.meshid, mtype: node.mtype, msgid: 133, msg: "Turn off." };
                                        parent.parent.DispatchEvent(targets, obj, event);
                                        return;
                                    }
                                    ...
                                    if ((command.actiontype >= 300) && (command.actiontype < 400)) {
                                        ...
                                        parent.parent.DispatchEvent('*', obj, { correlationid: command?.correlationid, action: 'amtpoweraction', userid: user._id, username: user.name, nodeids: [node._id], domain: domain.id, nolog: 1, actiontype: command.actiontype - 300 });
                                    } else {
                                        if ((command.actiontype < 2) && (command.actiontype > 4)) return; // Invalid action type.
                                        // Renovar Agent power command, get this device and send the power command
                                        const agent = parent.wsagents[node._id];
                                        if (agent != null) {
                                            try { agent.send(JSON.stringify({ correlationid: command?.correlationid, action: 'poweraction', actiontype: command.actiontype, userid: user._id, username: user.name, remoteaddr: req.clientIp })); } catch (ex) { }
                                        }
                                    }
                                }
                            });
                            ...
                            if (command.responseid != null) {
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'poweraction', responseid: command.responseid, result: 'ok' })); } catch (ex) { }
                            } else {
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'poweraction' })); } catch (ex) { }
                            }
                        }
                        break;
                    }
                case 'toast':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'toast', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        if (command.title == null) { command.title = "RenovarCentral"; }
                        ...
                        for (i in command.nodeids) {
                            ...
                            parent.GetNodeWithRights(domain, user, command.nodeids[i], function (node, rights, visible) {
                                ...
                                if ((rights & MESHRIGHT_CHATNOTIFY) == 0) {
                                    if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'toast', responseid: command.responseid, result: 'Access Denied' })); } catch (ex) { } }
                                    return;
                                }
                                ...
                                const agent = parent.wsagents[node._id];
                                if (agent != null) {
                                    try { agent.send(JSON.stringify({ correlationid: command?.correlationid, action: 'toast', title: command.title, msg: command.msg, sessionid: ws.sessionId, username: user.name, userid: user._id })); } catch (ex) { }
                                }
                            });
                        }
           
                        // Send response if required
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'toast', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'changedevice':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changedevice', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                            if ((rights & MESHRIGHT_MANAGECOMPUTERS) == 0) {
                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changedevice', responseid: command.responseid, result: 'Access Denied' })); } catch (ex) { } }
                                return;
                            }
                            var mesh = parent.meshes[node.meshid], amtchange = 0;
           
                            // Ready the node change event
                            var changes = [], event = { etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'changenode', nodeid: node._id, domain: domain.id };
                            change = 0;
                            event.msg = ': ';
                            ...
                            // Send response if required
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'changedevice', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        });
                        break;
                    }
                case 'uploadagentcore':
                    {
                        ...
                        for (var i in command.nodeids) {
                            ...
                            parent.GetNodeWithRights(domain, user, nodeid, function (node, rights, visible) {
                                ...
                                //if (parent.parent.multiServer != null) { parent.parent.multiServer.DispatchMessage({ correlationid: command?.correlationid, action: 'uploadagentcore', sessionid: ws.sessionId }); }
                ...
                case 'inviteAgent':
                    {
                        ...
                        // Handle any errors
                        if (err != null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'inviteAgent', responseid: command.responseid, result: err })); } catch (ex) { } }
                            break;
                        }
                        ...
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'inviteAgent', responseid: command.responseid, result: 'ok' })); } catch (ex) { } }
                        break;
                    }
                case 'setDeviceEvent':
                    {
                        ...
                        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                            ...
                            var targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, ['server-users', user._id]);
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, nodeid: node._id, correlationid: command?.correlationid, action: 'manual', msg: decodeURIComponent(command.msg), domain: domain.id };
                            parent.parent.DispatchEvent(targets, obj, event);
                        });
                        break;
                    }
                ...
                case 'otpemail':
                    {
                        ...
                        parent.db.SetUser(user);
                        ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpemail', success: true, enabled: command.enabled })); // Report success
           
                        // Notify change
                        var targets = ['*', 'server-users', user._id];
                        if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: command.enabled ? 88 : 89, msg: command.enabled ? "Enabled email two-factor authentication." : "Disabled email two-factor authentication.", domain: domain.id };
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                        break;
                    }
                case 'otpauth-request':
                    {
                        // Do not allow this command if 2FA's are locked
                        if ((domain.passwordrequirements) && (domain.passwordrequirements.lock2factor == true)) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', err: 1 })); return; }
           
                        // Do not allow this command when logged in using a login token
                        if (req.session.loginToken != null) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', err: 3 })); return; }
           
                        // Check of OTP 2FA is allowed
                        if ((domain.passwordrequirements) && (domain.passwordrequirements.otp2factor == false)) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', err: 4 })); return; }
           
                        if ((user.siteadmin != 0xFFFFFFFF) && ((user.siteadmin & 1024) != 0)) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', err: 5 })); return; } // If this account is settings locked, return here.
                        ...
                        if (twoStepLoginSupported) {
                            ...
                            if (otplib == null) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', err: 6 })); return; }
                            ...
                            ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-request', secret: secret, url: otplib.authenticator.keyuri(encodeURIComponent(user.name), domainName, secret) }));
                        }
                        break;
                    }
                case 'otpauth-setup':
                    {
                        ...
                        if (twoStepLoginSupported) {
                            ...
                            if (otplib.authenticator.check(command.token, command.secret) === true) {
                                ...
                                parent.db.SetUser(user);
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-setup', success: true })); // Report success
           
                                // Notify change
                                var targets = ['*', 'server-users', user._id];
                                if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 90, msg: 'Added authentication application', domain: domain.id };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                parent.parent.DispatchEvent(targets, obj, event);
                            } else {
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-setup', success: false })); // Report fail
                            }
                        }
                        break;
                    }
                case 'otpauth-clear':
                    {
                        ...
                        if (twoStepLoginSupported) {
                            ...
                            if (user.otpsecret) {
                                ...
                                parent.db.SetUser(user);
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-clear', success: true })); // Report success
           
                                // Notify change
                                var targets = ['*', 'server-users', user._id];
                                if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 91, msg: 'Removed authentication application', domain: domain.id };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                parent.parent.DispatchEvent(targets, obj, event);
                            } else {
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-clear', success: false })); // Report fail
                            }
                        }
                        break;
                    }
                case 'otpauth-getpasswords':
                    {
                        ...
                        if (count2factoraAuths() > 0) {
                            ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otpauth-getpasswords', passwords: user.otpkeys ? user.otpkeys.keys : null }));
                        }
                        ...
                        if (actionText != null) {
                            var targets = ['*', 'server-users', user._id];
                            if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: actionId, msg: actionText, domain: domain.id };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                            parent.parent.DispatchEvent(targets, obj, event);
                        }
                        break;
                ...
                case 'otp-hkey-get':
                    {
                        ...
                        ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-get', keys: hkeys }));
                        break;
                    }
                case 'otp-hkey-remove':
                    {
                        ...
                        var targets = ['*', 'server-users', user._id];
                        if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 94, msg: 'Removed security key', domain: domain.id };
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                        break;
                    }
                case 'otp-hkey-yubikey-add':
                    {
                        ...
                        if ((yubikeyotp == null) || (twoStepLoginSupported == false) || (typeof command.otp != 'string')) {
                            ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-yubikey-add', result: false, name: command.name }));
                            break;
                        }
                        ...
                        yubikeyotp.verifyOTP(request, function (err, results) {
                            if ((results != null) && (results.status == 'OK')) {
                                ...
                                parent.db.SetUser(user);
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-yubikey-add', result: true, name: command.name, index: keyIndex }));
           
                                // Notify change TODO: Should be done on all sessions/servers for this user.
                                var targets = ['*', 'server-users', user._id];
                                if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 95, msg: 'Added security key', domain: domain.id };
                                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                                parent.parent.DispatchEvent(targets, obj, event);
                            } else {
                                ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-yubikey-add', result: false, name: command.name }));
                ...
                case 'otpdev-clear':
                    {
                        ...
                        if (user.otpdev != null) {
                            ...
                            var targets = ['*', 'server-users', user._id];
                            if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', msgid: 114, msg: "Removed push notification authentication device", domain: domain.id };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                            parent.parent.DispatchEvent(targets, obj, event);
                        }
                        break;
                ...
                case 'otpdev-set':
                    {
                        ...
                        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                            ...
                            var payload = { notification: { title: "RenovarCentral", body: user.name + " authentication" }, data: { url: '2fa://auth?code=' + code + '&c=' + authCookie } };
                ...
                case 'webauthn-startregister':
                    {
                        ...
                        obj.webAuthnReqistrationRequest = { correlationid: command?.correlationid, action: 'webauthn-startregister', keyname: command.name, request: registrationOptions };
                ...
                case 'webauthn-endregister':
                    {
                        ...
                        if (regResult.verified === true) {
                            ...
                            parent.db.SetUser(user);
                            ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-setup-response', result: true, name: command.name, index: keyIndex }));
                            ...
                            ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'otp-hkey-setup-response', result: false, error: regResult.error, name: command.name, index: keyIndex }));
                        }
                ...
                case 'userWebState': {
                    ...
                    parent.parent.DispatchEvent([user._id], obj, { correlationid: command?.correlationid, action: 'userWebState', nolog: 1, domain: domain.id, state: command.state });
                    break;
                }
                case 'getNotes':
                    {
                        ...
                        if (idtype == 'node') {
                            ...
                            parent.GetNodeWithRights(domain, user, command.id, function (node, rights, visible) {
                                ...
                                db.Get('nt' + command.id, function (err, notes) {
                                    try {
                                        if ((notes == null) || (notes.length != 1)) { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'getNotes', id: command.id, notes: null })); return; }
                                        ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'getNotes', id: command.id, notes: notes[0].value }));
                                    } catch (ex) { }
                                });
                            });
                        } else if (idtype == 'mesh') {
                ...
                case 'createInviteLink': {
                    ...
                    if (err != null) {
                        ...
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createInviteLink', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                    ...
                    if (inviteCookie == null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createInviteLink', responseid: command.responseid, result: 'Unable to generate invitation cookie' })); } catch (ex) { } } break; }
                    ...
                    ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createInviteLink', meshid: command.meshid, url: url, expire: command.expire, cookie: inviteCookie, responseid: command.responseid, tag: command.tag }));
                    break;
                }
                case 'deviceMeshShares': {
                    ...
                    // Handle any errors
                    if (err != null) {
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deviceShares', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                    ...
                    parent.db.GetAllTypeNoTypeField('deviceshare', domain.id, function (err, docs) {
                        ...
                        for (var i = 0; i < docs.length; i++) {
                            ...
                            if ((doc.expireTime != null) && (doc.expireTime < now)) {
                                ...
                                parent.parent.DispatchEvent(targets, obj, { etype: 'node', meshid: doc.xmeshid, nodeid: doc.nodeid, correlationid: command?.correlationid, action: 'deviceShareUpdate', domain: domain.id, deviceShares: okDocs, nolog: 1 });
                            } else {
                                if (doc.xmeshid == null) {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deviceMeshShares', meshid: command.meshid, deviceShares: okDocs })); } catch (ex) { }
                    });
                    break;
                }
                case 'deviceShares': {
                    ...
                    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                        ...
                        if (node == null) {
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deviceShares', responseid: command.responseid, result: 'Invalid node id' })); } catch (ex) { } }
                            return;
                        }
                        ...
                        if (rights != MESHRIGHT_ADMIN) {
                            ...
                            if (((rights & MESHRIGHT_REMOTECONTROL) == 0) || ((rights & MESHRIGHT_DESKLIMITEDINPUT) != 0) || ((rights & MESHRIGHT_GUESTSHARING) == 0)) {
                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deviceShares', responseid: command.responseid, result: 'Access denied' })); } catch (ex) { } }
                                return;
                            }
                        }
                        ...
                        parent.db.GetAllTypeNodeFiltered([command.nodeid], domain.id, 'deviceshare', null, function (err, docs) {
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'deviceShares', nodeid: command.nodeid, deviceShares: okDocs })); } catch (ex) { }
           
                            // If we removed any shares, send device share update
                            if (removed == true) {
                                var targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, ['server-users', user._id]);
                                parent.parent.DispatchEvent(targets, obj, { etype: 'node', nodeid: node._id, correlationid: command?.correlationid, action: 'deviceShareUpdate', domain: domain.id, deviceShares: okDocs, nolog: 1 });
                            }
                        });
                    });
                ...
                case 'removeDeviceShare': {
                    ...
                    // Handle any errors
                    if (err != null) {
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removeDeviceShare', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                    ...
                    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                        ...
                        parent.db.GetAllTypeNodeFiltered([command.nodeid], domain.id, 'deviceshare', null, function (err, docs) {
                            ...
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removeDeviceShare', responseid: command.responseid, nodeid: command.nodeid, publicid: command.publicid, removed: removedExact })); } catch (ex) { } }
                            ...
                            if (removedExact != null) {
                                ...
                                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'node', userid: user._id, username: user.name, nodeid: node._id, correlationid: command?.correlationid || parent?.correlationid, action: 'removedDeviceShare', msg: 'Removed Device Share', msgid: 102, msgArgs: [removedExact.guestName], domain: domain.id, publicid: command.publicid };
                                parent.parent.DispatchEvent(targets, obj, event);
           
                                // If this is an agent self-sharing link, notify the agent
                                if (command.publicid.startsWith('AS:node/')) { routeCommandToNode({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'guestShare', nodeid: command.publicid.substring(3), flags: 0, url: null, viewOnly: false }); }
                            }
                            ...
                            if (removed == true) {
                                var targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, ['server-users', user._id]);
                                parent.parent.DispatchEvent(targets, obj, { etype: 'node', nodeid: node._id, correlationid: command?.correlationid, action: 'deviceShareUpdate', domain: domain.id, deviceShares: okDocs, nolog: 1 });
                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removeDeviceShare', responseid: command.responseid, result: 'OK' })); } catch (ex) { } }
                            } else {
                                if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'removeDeviceShare', responseid: command.responseid, result: 'Invalid device share identifier.' })); } catch (ex) { } }
                            }
                        });
                    });
                    break;
                ...
                case 'createDeviceShareLink': {
                    ...
                    // Handle any errors
                    if (err != null) {
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createDeviceShareLink', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                    ...
                    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                        ...
                        const inviteCookie = parent.parent.encodeCookie(cookie, parent.parent.invitationLinkEncryptionKey);
                        if (inviteCookie == null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createDeviceShareLink', responseid: command.responseid, result: 'Unable to generate shareing cookie' })); } catch (ex) { } } return; }
                        ...
                        if (command.recurring == 1) {
                            event = { etype: 'node', userid: user._id, username: user.name, meshid: node.meshid, nodeid: node._id, correlationid: command?.correlationid, action: 'addedDeviceShare', msg: 'Added device share ' + command.guestname + ' recurring daily.', msgid: 138, msgArgs: [command.guestname], domain: domain.id };
                        } else if (command.recurring == 2) {
                            event = { etype: 'node', userid: user._id, username: user.name, meshid: node.meshid, nodeid: node._id, correlationid: command?.correlationid, action: 'addedDeviceShare', msg: 'Added device share ' + command.guestname + ' recurring weekly.', msgid: 139, msgArgs: [command.guestname], domain: domain.id };
                        } else if ((startTime != null) && (expireTime != null)) {
                            event = { etype: 'node', userid: user._id, username: user.name, meshid: node.meshid, nodeid: node._id, correlationid: command?.correlationid, action: 'addedDeviceShare', msg: 'Added device share: ' + command.guestname + '.', msgid: 101, msgArgs: [command.guestname, 'DATETIME:' + startTime, 'DATETIME:' + expireTime], domain: domain.id };
                        } else {
                            event = { etype: 'node', userid: user._id, username: user.name, meshid: node.meshid, nodeid: node._id, correlationid: command?.correlationid, action: 'addedDeviceShare', msg: 'Added device share ' + command.guestname + ' with unlimited time.', msgid: 131, msgArgs: [command.guestname], domain: domain.id };
                        }
                        parent.parent.DispatchEvent(targets, obj, event);
                        ...
                        parent.db.GetAllTypeNodeFiltered([command.nodeid], domain.id, 'deviceshare', null, function (err, docs) {
                            ...
                            // Send device share update
                            var targets = parent.CreateNodeDispatchTargets(node.meshid, node._id, ['server-users', user._id]);
                            parent.parent.DispatchEvent(targets, obj, { etype: 'node', nodeid: node._id, correlationid: command?.correlationid, action: 'deviceShareUpdate', domain: domain.id, deviceShares: docs, nolog: 1 });
                        });
                    });
                    break;
                }
                case 'traceinfo': {
                    ...
                    if ((user.siteadmin === SITERIGHT_ADMIN) && (typeof command.traceSources == 'object')) {
                        ...
                        parent.parent.DispatchEvent(['*'], obj, { correlationid: command?.correlationid, action: 'traceinfo', userid: user._id, username: user.name, traceSources: command.traceSources, nolog: 1, domain: domain.id });
                    }
                    break;
                }
                case 'sendmqttmsg': {
                    ...
                    // Handle any errors
                    if (err != null) {
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'sendmqttmsg', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                ...
                case 'getmqttlogin': {
                    ...
                    // Handle any errors
                    if (err != null) { if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'getmqttlogin', responseid: command.responseid, result: err })); } catch (ex) { } } break; }
                    ...
                    parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
                        ...
                        if (rights == MESHRIGHT_ADMIN) {
                            ...
                            var r = { correlationid: command?.correlationid, action: 'getmqttlogin', responseid: command.responseid, nodeid: node._id, user: token.user, pass: token.pass };
                            ...
                            if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'getmqttlogin', responseid: command.responseid, result: 'Unable to perform this operation' })); } catch (ex) { } }
                        }
                    });
                    break;
                }
                ...
                case 'plugins': {
                    ...
                    parent.db.getPlugins(function(err, docs) {
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'updatePluginList', list: docs, result: err })); } catch (ex) { } 
                    });
                    break;
                }
                case 'pluginLatestCheck': {
                    ...
                    .then(function(latest) {
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'pluginVersionsAvailable', list: latest })); } catch (ex) { } 
                    });
                    break;
                }
                case 'addplugin': {
                    ...
                    try {
                        ...
                        .then(function(docs){
                            var targets = ['*', 'server-users'];
                            parent.parent.DispatchEvent(targets, obj, { correlationid: command?.correlationid, action: 'updatePluginList', list: docs });
                        })
                        .catch(function(err) {
                            if (typeof err == 'object') err = err.message;
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'pluginError', msg: err })); } catch (er) { }
                        }); 
                ...
                case 'installplugin': {
                    ...
                    parent.parent.pluginHandler.installPlugin(command.id, command.version_only, null, function(){
                        ...
                        var targets = ['*', 'server-users'];
                        parent.parent.DispatchEvent(targets, obj, { correlationid: command?.correlationid, action: 'pluginStateChange' });
                    });
                    break;
                }
                case 'disableplugin': {
                ...
                case 'getpluginversions': {
                    ...
                    .then(function (versionInfo) {
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'downgradePluginVersions', info: versionInfo, error: null })); } catch (ex) { } 
                    })
                    .catch(function (e) {
                      try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'pluginError', msg: e })); } catch (ex) { } 
                    });
                ...
                case 'uicustomevent': {
                    if ((command.src != null) && (Array.isArray(command.src.selectedDevices))) {
                        ...
                        parent.GetNodesWithRights(domain, user, command.src.selectedDevices, function (nodes) {
                            ...
                            var message = { etype: 'user', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'uicustomevent', domain: domain.id, uisection: command.section, element: command.element };
                ...
                case 'serverBackup': {
                    ...
                    if (command.service == 'googleDrive') {
                        if (command.state == 0) {
                            parent.db.Remove('GoogleDriveBackup', function () { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'serverBackup', service: 'googleDrive', state: 1 })); } catch (ex) { } });
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'serverBackup', service: 'googleDrive', state: 2, url: authUrl })); } catch (ex) { }
                            ...
                            obj.oAuth2Client.getToken(command.code, function (err, token) {
                                ...
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'serverBackup', service: 'googleDrive', state: 3 })); } catch (ex) { }
                            });
                        }
                    }
                    break;
                ...
                case 'twoFactorCookie': {
                    try {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'twoFactorCookie', cookie: twoFactorCookie })); } catch (ex) { }
                    } catch (ex) { console.log(ex); }
                    break;
                }
                case 'amtsetupbin': {
                    ...
                    if ((command.baremetal) && (parent.parent.amtProvisioningServer != null)) {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'amtsetupbin', file: Buffer.from(bin, 'binary').toString('base64') })); } catch (ex) { }
                    } else {
                ...
                case 'meshToolInfo': {
                    ...
                    var responseCmd = { correlationid: command?.correlationid, action: 'meshToolInfo', name: command.name, hash: info.hash, size: info.size, url: info.url };
                    ...
                    db.Get(command.nodeid, function (err, nodes) { // TODO: Make a NodeRights(user) method that also does not do a db call if agent is connected (???)
                        if ((err == null) && (nodes.length == 1)) {
                            ...
                            if ((parent.GetNodeRights(user, node.meshid, node._id) == MESHRIGHT_ADMIN) && (typeof node.pmt == 'string')) {
                                ...
                                parent.parent.firebase.sendToDevice(node, payload, options, function (id, err, errdesc) {
                                    if (err != null) {
                                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'console', nodeid: node._id, value: 'Failed: ' + errdesc })); } catch (ex) { }
                ...
                case 'webpush': {
                    ...
                    if (changed == true) {
                        ...
                        // Event the change
                        var message = { etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), correlationid: command?.correlationid, action: 'accountchange', domain: domain.id, nolog: 1 };
                    ...
                    if (command.userid == null) {
                        ...
                        if (db.GetUserLoginEvents) {
                            ...
                            db.GetUserLoginEvents(domain.id, user._id, function (err, docs) {
                                ...
                                try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'previousLogins', events: e })); } catch (ex) { }
                            });
                        } else {
                            // Old way
                            ...
                            if ((obj.crossDomain === true) || (splitUser[1] === domain.id)) {
                                if (db.GetUserLoginEvents) {
                                    ...
                                    db.GetUserLoginEvents(splitUser[1], command.userid, function (err, docs) {
                                        ...
                                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'previousLogins', userid: command.userid, events: e })); } catch (ex) { }
                                    });
                                } else {
                                    // Old way
                ...
                case 'oneclickrecovery': { // Intel(R) AMT One Click Recovery (OCR)
                    ...
                    for (var i = 0; i < command.nodeids.length; i++) {
                        ...
                        parent.GetNodeWithRights(domain, user, xnodeid, function (node, rights, visible) {
                            ...
                            parent.parent.DispatchEvent('*', obj, { correlationid: command?.correlationid, action: 'oneclickrecovery', userid: user._id, username: user.name, nodeids: [node._id], domain: domain.id, nolog: 1, file: file.fullpath });
                        });
                    }
                    break;
                }
                case 'loginTokens': { // Respond with the list of currently valid login tokens
                    ...
                    parent.db.GetAllTypeNodeFiltered(['logintoken-' + user._id], domain.id, 'logintoken', null, function (err, docs) {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'loginTokens', loginTokens: okDocs })); } catch (ex) { }
                        ...
                        if (removed.length > 0) {
                            ...
                            var targets = ['*', 'server-users', user._id];
                            if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'loginTokenChanged', domain: domain.id, loginTokens: okDocs, removed: removed, nolog: 1 };
                            parent.parent.DispatchEvent(targets, obj, event);
                        }
                    });
                    break;
                ...
                case 'createLoginToken': { // Create a new login token
                    ...
                    // Handle any errors
                    if (err != null) {
                        if (command.responseid != null) { try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createLoginToken', responseid: command.responseid, result: err })); } catch (ex) { } }
                        break;
                    }
                    ...
                    require('./pass').hash(tokenPass, function (err, salt, hash, tag) {
                        ...
                        try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'createLoginToken', name: command.name, tokenUser: tokenUser, tokenPass: tokenPass, created: created, expire: expire })); } catch (ex) { }
           
                        // Dispatch the new event
                        var targets = ['*', 'server-users', user._id];
                        if (user.groups) { for (var i in user.groups) { targets.push('server-users:' + i); } }
                        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'loginTokenAdded', msgid: 115, msg: "Added login token", domain: domain.id, newToken: { name: command.name, tokenUser: tokenUser, created: created, expire: expire } };
                        parent.parent.DispatchEvent(targets, obj, event);
                    });
                    break;
                }
                case 'getDeviceDetails': {
                    ...
                    db.GetAllIdsOfType(ids, domain.id, 'lastconnect', function (err, docs) {
                        ...
                        getDeviceDetailedInfo(command.nodeids, command.type, function (results, type) {
                            ...
                            try { ws.send(JSON.stringify({ correlationid: command?.correlationid, action: 'getDeviceDetails', data: output, type: type })); } catch (ex) { }
                        });
                    });
                ...
                case 'endDesktopMultiplex': {
                    ...
                    parent.GetNodeWithRights(domain, xuser, command.nodeid, function (node, rights, visible) {
                        ...
                        const event = { etype: 'user', userid: user._id, username: user.name, nodeid: command.nodeid, xuserid: command.xuserid, correlationid: command?.correlationid, action: 'endsession', msgid: 134, msgArgs: [xusername], msg: 'Forcibly disconnected desktop session of user ' + xusername, domain: domain.id };
                ...
                case 'satellite': {
                    // Command indicates this is a RenovarCentral Satellite session and what featues it supports
                    ...
                    if ((command.reqid != null) && (typeof command.satelliteFlags == 'number')) {
                        const event = { correlationid: command?.correlationid, action: 'satelliteResponse', subcorrelationid: command?.correlationid, action: command.subaction, reqid: command.reqid, response: command.response, satelliteFlags: command.satelliteFlags, nolog: 1 }
                ...
                case 'importamtdevices': {
                    ...
                    for (var i in amtDevices) {
                        ...
                        db.Set(amtDevices[i]);
                        // Event the new node
                        parent.parent.DispatchEvent(parent.CreateMeshDispatchTargets(command.meshid, [nodeid]), obj, { etype: 'node', userid: user._id, username: user.name, correlationid: command?.correlationid, action: 'addnode', node: parent.CloneSafeNode(amtDevices[i]), msgid: 84, msgArgs: [amtDevices[i].name, mesh.name], msg: 'Added device ' + amtDevices[i].name + ' to device group ' + mesh.name, domain: domain.id });
                    }
                    break;
                }
                default: {
    ...
    const serverUserCommands = {
        ...
        'info': [serverUserCommandInfo, "Returns the most immidiatly useful information about this server, including RenovarCentral and NodeJS versions. This is often information required to file a bug."],
    ...
    function serverCommandAddDeviceUser(command) {
        ...
        parent.GetNodeWithRights(domain, user, command.nodeid, function (node, rights, visible) {
            ...
            for (var i in command.userids) {
                ...
                if (newuser != null) {
                    ...
                    if (newuserid.startsWith('user/')) {
                        ...
                        var event;
                        if (command.rights == 0) {
                            event = {correlationid: event?.correlationid, etype: 'user', userid: user._id, username: user.name, action: 'accountchange', msgid: 81, msgArgs: [newuser.name], msg: 'Removed user device rights for ' + newuser.name, domain: domain.id, account: parent.CloneSafeUser(newuser), nodeListChange: newuserid };
                        } else {
                            event = {correlationid: event?.correlationid, etype: 'user', userid: user._id, username: user.name, action: 'accountchange', msgid: 82, msgArgs: [newuser.name], msg: 'Changed user device rights for ' + newuser.name, domain: domain.id, account: parent.CloneSafeUser(newuser), nodeListChange: newuserid };
                        }
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                        ...
                        var event = {correlationid: event?.correlationid, etype: 'ugrp', username: user.name, ugrpid: newuser._id, name: newuser.name, action: 'usergroupchange', links: newuser.links, msgid: 79, msgArgs: [newuser.name], msg: 'User group changed: ' + newuser.name, domain: domain.id };
                        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                        parent.parent.DispatchEvent(targets, obj, event);
                    }
                }
            ...
            if (nodeChanged == true) {
                ...
                var event;
                if (command.rights == 0) {
                    event = {correlationid: event?.correlationid, etype: 'node', userid: user._id, username: user.name, action: 'changenode', nodeid: node._id, domain: domain.id, msgid: 81, msgArgs: [node.name], msg: 'Removed user device rights for ' + node.name, node: parent.CloneSafeNode(node) };
                } else {
                    event = {correlationid: event?.correlationid, etype: 'node', userid: user._id, username: user.name, action: 'changenode', nodeid: node._id, domain: domain.id, msgid: 82, msgArgs: [node.name], msg: 'Changed user device rights for ' + node.name, node: parent.CloneSafeNode(node) };
                }
                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the mesh. Another event will come.
                parent.parent.DispatchEvent(dispatchTargets, obj, event);
            }
       
            if (command.responseid != null) { obj.send({correlationid: event?.correlationid, action: 'adddeviceuser', responseid: command.responseid, result: 'ok' }); }
        });
    }
        ...
        for (var i in command.userids) {
            ...
            if (newuser != null) {
                ...
                if (newuserid.startsWith('user/')) {
                    ...
                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(newuser), action: 'accountchange', msgid: 78, msgArgs: [newuser.name], msg: 'Device group membership changed: ' + newuser.name, domain: domain.id };
                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                    parent.parent.DispatchEvent(targets, obj, event);
                } else if (newuserid.startsWith('ugrp/')) {
                    // Notify user group change
                    var targets = ['*', 'server-ugroups', user._id, newuser._id];
                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', username: user.name, ugrpid: newuser._id, name: newuser.name, desc: newuser.desc, action: 'usergroupchange', links: newuser.links, msgid: 79, msgArgs: [newuser.name], msg: 'User group changed: ' + newuser.name, domain: domain.id };
                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                    parent.parent.DispatchEvent(targets, obj, event);
                }
        ...
        db.isMaxType(newuserdomain.limits.maxuseraccounts, 'user', newuserdomain.id, function (maxExceed) {
            if (maxExceed) {
                ...
                if (command.responseid != null) {
                    ...
                    // Create the notification message
                    var notification = {correlationid: command?.correlationid || event?.correlationid, action: 'msg', type: 'notify', id: Math.random(), value: "Account limit reached.", title: "Server Limit", userid: user._id, username: user.name, domain: newuserdomain.id, titleid: 2, msgid: 10 };
                ...
                if (typeof newuserdomain.newaccountsusergroups == 'object') {
                    for (var i in newuserdomain.newaccountsusergroups) {
                        ...
                        if (ugroup != null) {
                            ...
                            // Notify user group change
                            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'ugrp', userid: user._id, username: user.name, ugrpid: ugroup._id, name: ugroup.name, desc: ugroup.desc, action: 'usergroupchange', links: ugroup.links, msgid: 80, msgArgs: [newuser.name, ugroup.name], msg: 'Added user ' + newuser.name + ' to user group ' + ugroup.name, addUserDomain: newuserdomain.id };
                            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user group. Another event will come.
    ...
    function serverCommandAddUserBatch(command) {
        ...
        db.isMaxType(domain.limits.maxuseraccounts + userCount, 'user', domain.id, function (maxExceed) {
            if (maxExceed) {
                ...
                // Create the notification message
                var notification = {correlationid: command?.correlationid, action: 'msg', type: 'notify', id: Math.random(), value: "Account limit reached.", title: "Server Limit", userid: user._id, username: user.name, domain: domain.id, titleid: 2, msgid: 10 };
        ...
        if (group != null) {
            ...
            for (var i in command.usernames) {
                ...
                if (chguser != null) {
                    ...
                    // Notify user change
                    var targets = ['*', 'server-users', user._id, chguser._id];
                    var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(chguser), action: 'accountchange', msgid: 67, msgArgs: [chguser.name], msg: 'User group membership changed: ' + chguser.name, domain: addUserDomain.id };
                    if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                    parent.parent.DispatchEvent(targets, obj, event);
        ...
        if (obj.user.email != command.email) {
            ...
            db.GetUserWithVerifiedEmail(domain.id, command.email, function (err, docs) {
                if ((docs != null) && (docs.length > 0)) {
                    ...
                    obj.send({correlationid: command?.correlationid, action: 'msg', type: 'notify', title: 'Account Settings', id: Math.random(), tag: 'ServerNotify', value: 'Failed to change email address, another account already using: ' + command.email + '.', titleid: 4, msgid: 13, args: [command.email] });
                } else {
    ...
    function serverCommandConfirmPhone(command) {
        ...
        // Event the change
        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), action: 'accountchange', msgid: 96, msgArgs: [user.name], msg: 'Verified phone number of user ' + EscapeHtml(user.name), domain: domain.id };
        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
    ...
    function serverCommandConfirmMessaging(command) {
        ...
        // Event the change
        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), action: 'accountchange', msgid: 156, msgArgs: [user.name], msg: 'Verified messaging account of user ' + EscapeHtml(user.name), domain: domain.id };
        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
        parent.parent.DispatchEvent(['*', 'server-users', user._id], obj, event);
    }
    ...
    function serverCommandRemovePhone(command) {
        ...
        // Event the change
        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), action: 'accountchange', msgid: 97, msgArgs: [user.name], msg: 'Removed phone number of user ' + EscapeHtml(user.name), domain: domain.id };
        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
        parent.parent.DispatchEvent(['*', 'server-users', user._id], obj, event);
    }
    ...
    function serverCommandRemoveMessaging(command) {
        ...
        // Event the change
        var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(user), action: 'accountchange', msgid: 157, msgArgs: [user.name], msg: 'Removed messaging account of user ' + EscapeHtml(user.name), domain: domain.id };
        if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
        parent.parent.DispatchEvent(['*', 'server-users', user._id], obj, event);
    }
    ...
    function serverCommandRemoveUserFromUserGroup(command) {
        ...
        if (chguser != null) {
            ...
            if ((chguser.links != null) && (chguser.links[command.ugrpid] != null)) {
                ...
                // Notify user change
                var targets = ['*', 'server-users', user._id, chguser._id];
                var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: user._id, username: user.name, account: parent.CloneSafeUser(chguser), action: 'accountchange', msgid: 67, msgArgs: [chguser.name], msg: 'User group membership changed: ' + chguser.name, domain: removeUserDomain.id };
                if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
                parent.parent.DispatchEvent(targets, obj, event);
    ...
    function serverCommandUpdateAgents(command) {
        ...
        for (var i in command.nodeids) { routeCommandToNode({correlationid: command?.correlationid, action: 'msg', type: 'console', nodeid: command.nodeids[i], value: 'agentupdate' }, MESHRIGHT_ADMIN, 0); }
    }
    ...
    function serverCommandUpdateUserImage(command) {
        ...
        if (change == 1) {
            ...
            var allTargetGroups = chguser.groups;
            if (allTargetGroups) { for (var i in allTargetGroups) { targets.push('server-users:' + i); } }
            var event = { correlationid: command?.correlationid || event?.correlationid, etype: 'user', userid: uid, username: chguser.name, account: parent.CloneSafeUser(chguser), action: 'accountchange', msgid: 66, msgArgs: [chguser.name], msg: 'Account changed: ' + chguser.name, domain: domain.id, accountImageChange: 1 };
            if (db.changeStream) { event.noact = 1; } // If DB change stream is active, don't use this event to change the user. Another event will come.
            parent.parent.DispatchEvent(targets, obj, event);
        }
    }
    ...
    function remoteSessionReport(command, manageAllDeviceGroups, msgIdFilter) {
        ...
        db.GetEventsTimeRange(ids, domain.id, msgIdFilter, new Date(command.start * 1000), new Date(command.end * 1000), function (err, docs) {
            ...
            try { ws.send(JSON.stringify({correlationid: command?.correlationid, action: 'report', data: data })); } catch (ex) { }
        });
    }
        ...
        switch (cmd.command) {
            case 'Get-GeneralSettings': {
                amt.Get('AMT_GeneralSettings', function (obj, name, response, status) {
                    if (status == 200) {
                        var resp = {correlationid: cmd?.correlationid, action: 'amt', nodeid: cmd.nodeid, command: 'Get-GeneralSettings', value: response.Body }
