/**
* @description RenovarCentral Intel(R) AMT MPS server
...
module.exports.CreateMpsServer = function (parent, db, args, certificates) {
    ...
    obj.httpResponses = {
        '/': '<!DOCTYPE html><html><head><meta charset=\"UTF-8\"></head><body>RenovarCentral MPS server.<br />Intel&reg; AMT computers should connect here.</body></html>'
    ...
    if ((args.lanonly != true) && (args.mpsport !== 0)) {
        ...
        obj.server.listen(args.mpsport, args.mpsportbind, function () {
            console.log("RenovarCentral Intel(R) AMT server running on " + certificates.AmtMpsName + ":" + args.mpsport + ((args.mpsaliasport != null) ? (", alias port " + args.mpsaliasport) : "") + ".");
            obj.parent.authLog('mps', 'Server listening on ' + ((args.mpsportbind != null) ? args.mpsportbind : '0.0.0.0') + ' port ' + args.mpsport + '.');
        }).on("error", function (err) { console.error("ERROR: RenovarCentral Intel(R) AMT server port " + args.mpsport + " is not available. Check if the RenovarCentral is already running."); if (args.exactports) { process.exit(); } });
