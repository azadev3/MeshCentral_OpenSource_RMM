/**
* @description RenovarCentral e-mail server communication modules
    ...
    obj.sendAccountLoginMail = function (domain, email, token, language, loginkey) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                // Set all the options.
                var options = { email: email, servername: domain.title ? domain.title : 'RenovarCentral', token: token };
    ...
    obj.sendAccountInviteMail = function (domain, username, accountname, email, password, language, loginkey) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                // Set all the options.
                var options = { username: username, accountname: accountname, email: email, servername: domain.title ? domain.title : 'RenovarCentral', password: password };
                if (loginkey != null) { options.urlargs1 = '?key=' + loginkey; options.urlargs2 = '&key=' + loginkey; } else { options.urlargs1 = ''; options.urlargs2 = ''; }
    ...
    obj.sendAccountCheckMail = function (domain, username, userid, email, language, loginkey) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                // Set all the options.
                var options = { username: username, email: email, servername: domain.title ? domain.title : 'RenovarCentral' };
                if (loginkey != null) { options.urlargs1 = '?key=' + loginkey; options.urlargs2 = '&key=' + loginkey; } else { options.urlargs1 = ''; options.urlargs2 = ''; }
    ...
    obj.sendAgentInviteMail = function (domain, username, email, meshid, name, os, msg, flags, expirehours, language, loginkey) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                var options = { username: username, name: name, email: email, installflags: flags, msg: msg, meshid: meshid, meshidhex: meshid.split('/')[2], servername: domain.title ? domain.title : 'RenovarCentral' };
                if (loginkey != null) { options.urlargs1 = '?key=' + loginkey; options.urlargs2 = '&key=' + loginkey; } else { options.urlargs1 = ''; options.urlargs2 = ''; }
    ...
    obj.sendDeviceNotifyMail = function (domain, username, email, connections, disconnections, language, loginkey) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                // Set all the template replacement options and generate the final email text (both in txt and html formats).
                const optionsHtml = { username: EscapeHtml(username), email: EscapeHtml(email), servername: EscapeHtml(domain.title ? domain.title : 'RenovarCentral'), header: true, footer: false };
                const optionsTxt = { username: username, email: email, servername: domain.title ? domain.title : 'RenovarCentral', header: true, footer: false };
    ...
    obj.sendDeviceHelpMail = function (domain, username, email, devicename, nodeid, helpusername, helprequest, language) {
        obj.checkEmail(email, function (checked) {
            if (checked) {
                ...
                // Set all the template replacement options and generate the final email text (both in txt and html formats).
                const optionsHtml = { devicename: EscapeHtml(devicename), helpusername: EscapeHtml(helpusername), helprequest: EscapeHtml(helprequest), nodeid: nodeid.split('/')[2], servername: EscapeHtml(domain.title ? domain.title : 'RenovarCentral') };
                const optionsTxt = { devicename: devicename, helpusername: helpusername, helprequest: helprequest, nodeid: nodeid.split('/')[2], servername: domain.title ? domain.title : 'RenovarCentral' };
